...
export async function subscribePush(token: string): Promise<{ ok: true } | { ok: false; reason: string }> {
...
export async function unsubscribePush(token: string): Promise<boolean> {
  ...
  await fetch(`/api/api/notifications/push-subscriptions?endpoint=${encodeURIComponent(endpoint)}`, {
    method: 'DELETE',
