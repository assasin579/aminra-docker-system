...
export default function NotificationBell({ token }: { token: string }) {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/api/notifications/?page=1&limit=10", {
        headers,
      });
      if (res.ok) {
        const data = await res.json();
        ...
        >
          ...
          >
            ...
            {unreadCount > 0 && (
              <button
                onClick={markAllRead}
                className="text-xs text-[#C9A24A] hover:text-[#C9A24A] transition-colors"
              >
              ...
              notifications.map((n, i) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-sm truncate ${!n.read ? "text-white font-bold" : "text-slate-400"}`}
                    >
                      {n.title}
                    </p>
                    <p
                      className={`text-xs truncate mt-0.5 ${!n.read ? "text-slate-300" : "text-slate-500"}`}
                    >
                      {n.message}
                    </p>
                  ...
                  {!n.read && (
                    <span className="mt-1.5 w-2 h-2 rounded-full bg-[#C9A24A] flex-shrink-0" />
                  )}
                </button>
              ))
            )}
