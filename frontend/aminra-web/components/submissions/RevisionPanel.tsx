...
export default function RevisionPanel({
  submissionId, status, role, documents, token, onChanged,
}: Props) {
  ...
  useEffect(() => { fetchHistory(); }, [submissionId]);

  // Determine what the panel can do based on role + status
  const providerCanRequest = role === 'provider' && (status === 'reviewing' || status === 'returned');
  const businessCanResubmit = role === 'business' && status === 'revision_required';
  ...
  return (
    <section className="rounded-2xl bg-white p-5"
      ...
      {error && (
        <div role="alert" className="bg-red-50 border border-red-200 rounded p-3 text-sm mb-3"
          style={{ color: '#991b1b' }}>
          {error}
        </div>
      )}
      ...
      {providerCanRequest && (
        <RequestRevisionForm
          ...
          documents={documents}
          token={token}
          onRequested={() => { fetchHistory(); onChanged?.(); }}
        />
      )}
      ...
      {businessCanResubmit && (
        <ResubmitForm
          ...
          documents={documents}
          token={token}
          onResubmitted={() => { fetchHistory(); onChanged?.(); }}
        />
      )}

      {/* Contextual hint when no form available but workflow could unlock it */}
      {!hasAction && hasHistory && role === 'provider' && (status === 'pending' || status === 'assigned') && (
        <div className="rounded-lg p-3 text-xs mb-4"
          ...
          Để yêu cầu sửa, đầu tiên chuyển trạng thái hồ sơ sang <strong>“Đang đánh giá”</strong>.
        </div>
      )}

      {/* History — visible to both */}
      <RevisionHistory history={history} loading={loading} documents={documents} />
    </section>
  );
}
...
function RequestRevisionForm({
  submissionId, documents, token, onRequested,
}: {
  submissionId: string;
  documents: DocumentInfo[];
  token: string;
  ...
  const updateDocIssue = (i: number, patch: Partial<DocumentFeedbackItem>) => {
    setDocFeedback(docFeedback.map((d, idx) => idx === i ? { ...d, ...patch } : d));
  };
      ...
      const res = await fetch(
        `/api/api/submissions/received/${submissionId}/request-revision`,
        {
          ...
          body: JSON.stringify({
            ...
            document_feedback: docFeedback.filter(d => d.issue.trim() && d.document_id),
          }),
        },
      );
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3 mb-5 pb-5"
      ...
      {docFeedback.map((d, i) => (
        <div key={i} className="rounded-lg p-3" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
          <div className="flex gap-2 mb-2">
            <select
              value={d.document_id}
              onChange={e => updateDocIssue(i, { document_id: e.target.value })}
            ...
            <select
              value={d.severity}
              onChange={e => updateDocIssue(i, { severity: e.target.value as Severity })}
            ...
            <button type="button" onClick={() => removeDocIssue(i)}
...
function ResubmitForm({
  submissionId, documents, token, onResubmitted,
}: {
  submissionId: string;
  documents: DocumentInfo[];
  token: string;
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `/api/api/submissions/${submissionId}/resubmit`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3 mb-5 pb-5"
      ...
      <div className="bg-amber-50 border border-amber-200 rounded p-3 text-sm"
        style={{ color: '#7c2d12' }}>
        Hồ sơ đang ở trạng thái <strong>cần sửa</strong>. Hãy upload tài liệu mới
        (nếu cần) và gửi lại để CB tiếp tục đánh giá.
      </div>
      <textarea
        value={businessNotes}
...
function RevisionHistory({
  ...
  if (loading) return <p className="text-sm" style={{ color: '#94A3B8' }}>Đang tải…</p>;
  if (history.length === 0) {
    return (
        ...
        <div key={r.id} className="rounded-lg p-3"
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs" style={{ color: '#94A3B8' }}>
              {new Date(r.requested_at).toLocaleString('vi-VN', { hour12: false })}
            </span>
          </div>
            ...
            <ul className="space-y-1.5 mt-2 ml-2">
              {r.document_feedback.map((d, i) => (
                <li key={i} className="text-xs" style={{ color: '#374151' }}>
                  <span className="font-mono px-1.5 py-0.5 rounded mr-1.5"
                    style={{
                      background: d.severity === 'critical' ? '#dc2626' :
                                  d.severity === 'major'    ? '#f59e0b' : '#94A3B8',
                      color: 'white',
                      fontSize: '10px',
                    }}>
                    {d.severity}
