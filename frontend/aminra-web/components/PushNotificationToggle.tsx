    ...
    if (r.ok) setState("enabled");
      ...
      setError(
        r.reason === "permission-denied"
          ...
          : `Không bật được thông báo (${r.reason})`,
      );
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs mt-0.5" style={{ color: "#64748B" }}>
            {state === "enabled" &&
              "Đang bật — bạn sẽ nhận thông báo về hồ sơ, chứng nhận, kiểm định ngay cả khi không mở web"}
            {state === "idle" &&
              "Bật để nhận thông báo realtime về thay đổi trạng thái hồ sơ + cert sắp hết hạn"}
            {state === "denied" &&
              "Bạn đã từ chối — vào cài đặt trình duyệt để bật lại quyền thông báo"}
          </p>
          {error && (
