          ...
          {data.valid ? (
            ...
            <>
              ...
              {data.status === 'revoked' && data.revocation?.reason && (
                <div className="mt-4 px-4 py-3 rounded-lg text-left text-sm"
                  style={{ background: 'rgba(220,38,38,0.08)', border: '1px solid rgba(220,38,38,0.2)' }}
                  data-revocation>
                  <p className="font-semibold mb-1" style={{ color: '#7f1d1d' }}>Lý do thu hồi:</p>
              ...
              <div className="px-5 py-4 space-y-2">
                <div className="flex items-center gap-2">
                  <span className="px-2 py-0.5 rounded-full text-xs font-semibold"
                    ...
                    ✓ Anchored on {data.blockchain.chain === 'polygon' ? 'Polygon' : 'Bitcoin'}
                  </span>
                ...
                <div className="text-xs space-y-1.5" style={{ color: '#6B7280' }}>
                  <div>
                    ...
                    <span className="font-mono">{data.blockchain.block_number?.toLocaleString()}</span>
                  </div>
                  <div className="break-all">
                    <span style={{ color: '#94A3B8' }}>Tx:</span>{' '}
                    <span className="font-mono">
                      {data.blockchain.tx_hash?.slice(0, 16)}…{data.blockchain.tx_hash?.slice(-8)}
                    </span>
                  </div>
                </div>
                {data.blockchain.explorer_url && (
                  <a href={data.blockchain.explorer_url} target="_blank" rel="noopener noreferrer"
                    className="inline-block mt-2 px-4 py-2 rounded-lg text-xs font-semibold"
                    style={{ background: '#7c3aed', color: 'white' }}>
                    Verify on {data.blockchain.chain === 'polygon' ? 'PolygonScan' : 'Blockchain.com'} →
                  </a>
                )}
...
{JSON.stringify({
  ...
  expected_root: data.blockchain.merkle_root,
              ...
              <div className="px-5 py-4 text-xs" style={{ color: '#6B7280' }}>
                ⏳ {data.blockchain.note || 'Anchor pending — will be confirmed within 24 hours.'}
              </div>
            )}
          </div>
        )}
