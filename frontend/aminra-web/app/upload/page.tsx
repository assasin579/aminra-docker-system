...
const DOC_TYPE_OPTIONS = [
  ...
  { id: 'internal_halal_committee', label: 'Internal Halal Committee',               group: 'policy' },
  { id: 'company_profile',          label: 'Company Profile',                        group: 'policy' },
  { id: 'ingredient_raw_material',  label: 'Ingredient & Raw Material Documentation', group: 'tech' },
...
function statusLabel(s: string) {
  if (s === 'compliant')     return { label: 'Tuân thủ',       color: '#0A1F44', bg: 'rgba(10,31,68,0.08)',    border: 'rgba(10,31,68,0.25)'   };
...
function severityBadge(sev: string) {
  const map: Record<string, { color: string; bg: string; label: string }> = {
    critical: { color: '#ef4444', bg: 'rgba(239,68,68,0.15)',  label: 'Nghiêm trọng' },
...
export default function UploadPage() {
  ...
  const [rewrites, setRewrites] = useState<Record<string, { rewritten: string; explanation: string; standards_referenced: string[] }>>({});
  ...
  const saveFileVersion = (r: EvaluationReport, prevVersions: FileVersionEntry[]) => {
    ...
    try {
      localStorage.setItem(`aminra_file_versions_${key}`, JSON.stringify(updated));
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    if (prevReport) {
      evalForm.append('previous_context', JSON.stringify({
        ...
        previous_issues_summary: prevReport.issues.slice(0, 5).map((i: EvalIssue) => i.issue).join('; '),
      }));
    }
    try {
      ...
      const res = await fetch('/api/evaluate', { method: 'POST', headers: uploadHeaders, body: evalForm });
      if (!res.ok) {
      ...
      setTimeout(() => {
        ...
        fetch(`${API}/reviews/${encodeURIComponent(file.name)}`)
          .then(r => r.json())
          .then(d => { if (d.exists && d.review) setExistingReview(d.review); })
          .catch(() => {});
      }, 400);
    } catch (e) {
      clearInterval(analyzeTimerRef.current!);
  ...
  const suggestRewrite = async (key: string, sectionText: string, issue: string) => {
    setRewriteLoading(key);
    try {
      const res = await fetch(`${API}/rewrite`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ section_text: sectionText, issue, doc_type: report?.doc_type ?? 'general' }),
      });
      const data = await res.json();
      setRewrites(prev => ({ ...prev, [key]: { rewritten: data.rewritten, explanation: data.explanation, standards_referenced: data.standards_referenced ?? [] } }));
    } catch {
      setRewrites(prev => ({ ...prev, [key]: { rewritten: 'Không thể tạo gợi ý lúc này.', explanation: '', standards_referenced: [] } }));
    } finally {
  ...
  const saveReview = async () => {
    ...
    try {
      await fetch(`${API}/reviews/${encodeURIComponent(fileName)}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...review, filename: fileName, reviewed_at: new Date().toISOString() }),
      });
      setReviewSaved(true);
    } catch {}
  };
    ...
    return (
      <div className="flex flex-col flex-1 lg:min-h-0 w-full" data-page>
        <div className="mb-6 animate-section">
          <h1 className="text-2xl font-bold" style={{ color: '#0A1F44' }}>Phân tích tài liệu Halal</h1>
          <p className="mt-2" style={{ color: '#6B7280' }}>Upload tài liệu để nhận đánh giá compliance toàn diện</p>
        </div>

        {/* Coming soon toast */}
        {showComingSoon && (
          <div className="mb-4 p-3 rounded-xl text-sm text-center" style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', color: '#f59e0b' }}>
            Tính năng này sẽ được cập nhật sớm nhất
          </div>
        )}

        {/* ── Step 1: Document type selector ─────────────────────────────── */}
        <div className="mb-6 rounded-2xl p-5" style={{ background: '#F5F1E8', border: '1px solid #E2E8F0' }}>
          <div className="grid grid-flow-col items-center gap-2 mb-4 justify-start">
            <span className="w-6 h-6 rounded-full grid place-items-center text-xs font-bold"
              style={{ background: selectedDocType ? '#0A1F44' : '#E2E8F0', color: selectedDocType ? 'white' : '#6B7280' }}>
              {selectedDocType ? '✓' : '1'}
            </span>
            <h2 className="text-sm font-semibold" style={{ color: '#0A1F44' }}>Chọn loại tài liệu</h2>
            {selectedDocType && (
              <span className="ml-auto text-xs px-2 py-0.5 rounded-full"
                style={{ background: 'rgba(10,31,68,0.08)', color: '#0A1F44', border: '1px solid rgba(10,31,68,0.2)' }}>
            ...
            {DOC_TYPE_OPTIONS.map(opt => {
              ...
              return (
                <button key={opt.id}
                  ...
                  style={{
                    background: selectedDocType === opt.id ? 'rgba(10,31,68,0.08)' : '#FFFFFF',
                    border: selectedDocType === opt.id ? '1px solid rgba(10,31,68,0.3)' : '1px solid #E2E8F0',
                    color: locked ? '#94A3B8' : selectedDocType === opt.id ? '#0A1F44' : '#6B7280',
          ...
          <div>
            <button
              ...
              style={{
                ...
                background: SOP_SUB_OPTIONS.some(o => o.id === selectedDocType) ? 'rgba(10,31,68,0.06)' : '#FFFFFF',
                border: SOP_SUB_OPTIONS.some(o => o.id === selectedDocType) ? '1px solid rgba(10,31,68,0.25)' : '1px solid #E2E8F0',
                color: !isFullAccess ? '#94A3B8' : SOP_SUB_OPTIONS.some(o => o.id === selectedDocType) ? '#0A1F44' : '#6B7280',
        ...
        <div className="mb-1 flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-[#0A1F44]">Tải lên tài liệu</h2>
        </div>

        {error && (
          <div className="mb-4 p-4 rounded-xl text-sm" style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', color: '#ef4444' }}>
            {error}
          </div>
        )}
        ...
        <div {...getRootProps()} className={`border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all grid place-items-center ${
          ...
          isDragActive ? 'border-[#0A1F44] bg-[#DCE3F0]' : 'border-[#E2E8F0] hover:border-[#0A1F44] bg-[#FFFFFF]'
        }`}>
          <input {...getInputProps()} />
          {phase === 'uploading' ? (
            <>
            ...
            <>
              <div className="h-16 w-16 rounded-full grid place-items-center mb-4" style={{ background: 'rgba(10,31,68,0.12)' }}>
                <svg className="w-8 h-8" style={{ color: '#0A1F44' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                </svg>
              </div>
              <h3 className="text-lg font-semibold text-[#0A1F44] mb-1">{isDragActive ? 'Thả file vào đây' : 'Kéo & thả file vào đây'}</h3>
              <p className="text-[#6B7280] text-sm mb-5">PDF, DOCX, PPTX, TXT, MD — tối đa 20 MB</p>
              <button type="button" className="px-6 py-2.5 font-medium rounded-xl transition-all" style={{ background: '#0A1F44', color: 'white' }}>
                Chọn file
              </button>
            </>
          )}
                ...
                return (
                  <div key={i} className="grid items-center gap-3 px-4 py-3" style={{ gridTemplateColumns: 'auto 1fr', borderBottom: i < Math.min(4, fileVersions.length - 1) ? '1px solid #E2E8F0' : 'none' }}>
                    ...
                    <div className="min-w-0">
                      <p className="text-sm text-[#0A1F44] truncate">v{v.version} · {v.doc_type_label}</p>
      ...
      <div className="flex-1 lg:min-h-0 w-full grid place-items-center"><div className="w-full text-center">
        <div className="mb-6 relative w-fit mx-auto">
          <svg width="120" height="120" viewBox="0 0 120 120">
            <circle cx="60" cy="60" r="50" fill="none" stroke="#DCE3F0" strokeWidth="8" />
            <circle cx="60" cy="60" r="50" fill="none" stroke="#0A1F44" strokeWidth="8"
          ...
          <div className="absolute inset-0 grid place-items-center">
            <span className="text-2xl font-bold" style={{ color: '#0A1F44' }}>{analyzeProgress}%</span>
          </div>
        </div>
        <h2 className="text-xl font-bold mb-2" style={{ color: '#0A1F44' }}>Đang phân tích tài liệu…</h2>
        <p className="text-sm mb-1" style={{ color: '#6B7280' }}>AI đang đọc và đối chiếu với tiêu chuẩn JAKIM/HDC</p>
        {previousReport && (
          <p className="text-xs mb-6" style={{ color: '#0A1F44' }}>↺ Phát hiện phiên bản trước (điểm {previousReport.compliance_score}) · Đang so sánh thay đổi</p>
        )}
    ...
    <div className="flex flex-col flex-1 lg:min-h-0 w-full" data-page>
      ...
      {isFullAccess && (
        <div className="grid items-center mb-4 px-4 py-2.5 rounded-xl text-sm"
          style={{ gridTemplateColumns: '1fr auto', background: 'rgba(34,197,94,0.07)', border: '1px solid rgba(10,31,68,0.15)' }}>
          <span style={{ color: '#0A1F44' }}>✓ Tài liệu đã được lưu và đánh giá thành công</span>
          {isBusiness && (
            <Link href="/documents" className="text-xs font-medium hover:underline" style={{ color: '#0A1F44' }}>
              Xem tất cả tài liệu →
            </Link>
          )}
        </div>
      ...
      <div className="grid items-start mb-6 gap-3" style={{ gridTemplateColumns: '1fr auto' }}>
        <div>
          <h1 className="text-xl font-bold text-[#0A1F44]">{report.filename}</h1>
        ...
        <button
          ...
          style={{ gridTemplateColumns: 'auto 1fr', background: '#0A1F44', color: 'white', boxShadow: '0 4px 12px rgba(10,31,68,0.3)' }}>
      ...
      {(() => {
        ...
        return (
          <div style={{
            ...
            background: '#F5F1E8', border: '1px solid #E2E8F0',
          }}>
            {visibleTabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
                ...
                boxShadow: activeTab === tab.id ? '0 2px 8px rgba(10,31,68,0.25)' : 'none',
              }}>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-5">
          {/* Score + Status */}
          <div className="rounded-2xl p-6 grid gap-6 items-center" style={{ gridTemplateColumns: 'auto 1fr', background: '#F5F1E8', border: '1px solid #E2E8F0' }}>
            ...
            <div className="min-w-0">
              ...
              <p className="text-[#0A1F44] text-base leading-relaxed">{report.summary}</p>
            </div>
          </div>

          {/* Strengths */}
          {report.strengths.length > 0 && (
            <div className="rounded-xl p-5" style={{ background: 'rgba(10,31,68,0.06)', border: '1px solid rgba(10,31,68,0.15)' }}>
              <h3 className="text-sm font-semibold mb-3" style={{ color: '#0A1F44' }}>✓ Điểm mạnh</h3>
              <ul className="space-y-2">
                {report.strengths.map((s, i) => (
                  <li key={i} className="grid gap-2 text-sm text-[#0A1F44]" style={{ gridTemplateColumns: 'auto 1fr' }}><span style={{ color: '#0A1F44' }}>·</span>{s}</li>
                ))}
              </ul>
            </div>
          )}
          ...
          {previousReport && (
            <div className="rounded-xl p-5" style={{ background: 'rgba(10,31,68,0.06)', border: '1px solid rgba(10,31,68,0.2)' }}>
              ...
              <div className="grid grid-flow-col items-center gap-6 mb-3 justify-start">
                ...
                {report.compliance_score !== previousReport.compliance_score && (
                  <span className="text-sm font-bold" style={{ color: report.compliance_score > previousReport.compliance_score ? '#0A1F44' : '#ef4444' }}>
              ...
              <div className="flex flex-wrap gap-4 text-xs" style={{ color: '#6B7280' }}>
                <span>Vấn đề: <strong style={{ color: report.issues.length < previousReport.issues.length ? '#0A1F44' : report.issues.length > previousReport.issues.length ? '#ef4444' : '#6B7280' }}>{previousReport.issues.length} → {report.issues.length}</strong></span>
          ...
          {report.issues.length > 0 && (
            <div className="rounded-xl p-5" style={{ background: '#F5F1E8', border: '1px solid #E2E8F0' }}>
              <h3 className="text-sm font-semibold mb-3 text-[#0A1F44]">Vấn đề phát hiện ({report.issues.length})</h3>
                ...
                {report.issues.map((issue, i) => {
                  ...
                  return (
                    <div key={i} className="rounded-xl p-4" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
                      ...
                      <p className="text-sm text-[#0A1F44] mt-2">{issue.issue}</p>
                      <p className="text-xs mt-1.5" style={{ color: '#6B7280' }}>💡 {issue.recommendation}</p>
                      {issue.reference && <p className="text-xs mt-1" style={{ color: '#0A1F44' }}>§ {issue.reference}</p>}
                      {/* F-06: Rewrite button */}
                      {!rewrites[key] && (
                        <button onClick={() => suggestRewrite(key, issue.issue + ' ' + issue.recommendation, issue.issue)}
                          ...
                          style={{ background: 'rgba(10,31,68,0.1)', color: '#6B7280', border: '1px solid rgba(10,31,68,0.2)' }}>
                          {rewriteLoading === key ? '⏳ Đang tạo gợi ý…' : '✨ Gợi ý viết lại'}
                        </button>
                      )}
                      {rewrites[key] && (
                        <div className="mt-3 rounded-lg p-3" style={{ background: 'rgba(10,31,68,0.08)', border: '1px solid rgba(10,31,68,0.2)' }}>
                          <p className="text-xs font-medium mb-1" style={{ color: '#6B7280' }}>✨ Gợi ý viết lại:</p>
                          <p className="text-sm text-[#0A1F44] leading-relaxed">{rewrites[key].rewritten}</p>
                          ...
                          {rewrites[key].standards_referenced?.length > 0 && (
                            <p className="text-xs mt-1" style={{ color: '#0A1F44' }}>§ {rewrites[key].standards_referenced.join(', ')}</p>
                          )}
                        </div>
                      )}
                    </div>
          ...
          {report.standards_checked.length > 0 && (
            <div className="rounded-xl p-5" style={{ background: '#F5F1E8', border: '1px solid #E2E8F0' }}>
              <h3 className="text-sm font-semibold mb-3 text-[#0A1F44]">Tiêu chuẩn đã đối chiếu</h3>
              <div className="flex flex-wrap gap-2">
                {report.standards_checked.map((s, i) => (
                  <span key={i} className="text-xs px-2.5 py-1 rounded-full"
                    style={{ background: 'rgba(10,31,68,0.06)', color: '#0A1F44', border: '1px solid rgba(10,31,68,0.15)' }}>
                    ✓ {s}
                  </span>
                ))}
              </div>
          ...
          {(report as any).signature_detection && (() => {
            ...
            const styles: Record<string, { icon: string; color: string; bg: string; border: string; label: string }> = {
              confirmed:   { icon: '✓', color: '#0A1F44', bg: 'rgba(10,31,68,0.06)', border: 'rgba(10,31,68,0.25)', label: 'Đã xác nhận' },
              likely:      { icon: '?', color: '#6B7280', bg: 'rgba(10,31,68,0.06)', border: 'rgba(10,31,68,0.3)', label: 'Cần xác nhận' },
            ...
            return (
              <div className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${statusStyle.border}` }}>
                <div className="px-5 py-4" style={{ background: statusStyle.bg, borderBottom: '1px solid rgba(226,232,240,0.6)' }}>
                  <div className="grid grid-flow-col items-center gap-3 justify-start">
                    ...
                    <div>
                      <div className="grid grid-flow-col items-center gap-2 justify-start">
                        <h3 className="text-sm font-semibold text-[#0A1F44]">Chữ ký & Con dấu</h3>
                  ...
                  <div className="px-5 py-3 space-y-1.5">
                    {sig.signature_zones.slice(0, 8).map((z: any, i: number) => (
                      <div key={i} className="grid grid-flow-col items-center gap-2 justify-start text-xs">
                        <span style={{ color: z.type === 'image' ? '#0A1F44' : z.type === 'text_pattern' ? '#6B7280' : '#fbbf24' }}>
          ...
          {!report.gap_analysis ? (
            ...
            <>
              {[
                ...
                return (
                  <div key={group.key} className="rounded-xl p-5" style={{ background: group.bg, border: `1px solid ${group.border}` }}>
                    <h3 className="text-sm font-semibold mb-3 flex items-center gap-2" style={{ color: group.color }}>
                      <span>{group.icon}</span>{group.label} ({items.length})
                    </h3>
                    <ul className="space-y-2">
                      {items.map((gap, i) => {
                          ...
                          <li key={i} className="rounded-lg p-3" style={{ background: 'rgba(0,0,0,0.03)' }}>
                            ...
                            {!rewrites[rewriteKey] && (
                              <button onClick={() => suggestRewrite(rewriteKey, gap, gap)}
                                ...
                                style={{ background: 'rgba(10,31,68,0.1)', color: '#6B7280', border: '1px solid rgba(10,31,68,0.15)' }}>
                                {rewriteLoading === rewriteKey ? '⏳ Đang tạo…' : '✨ Gợi ý khắc phục'}
                              </button>
                            )}
                            {rewrites[rewriteKey] && (
                              <div className="mt-2 rounded-lg p-2.5" style={{ background: 'rgba(10,31,68,0.08)', border: '1px solid rgba(10,31,68,0.2)' }}>
                                <p className="text-xs font-medium mb-1" style={{ color: '#6B7280' }}>✨ Gợi ý:</p>
                                <p className="text-sm text-[#0A1F44]">{rewrites[rewriteKey].rewritten}</p>
                              </div>
                            )}
                          </li>
                        );
              ...
              {report.recommendations.length > 0 && (
                <div className="rounded-xl p-5" style={{ background: '#F5F1E8', border: '1px solid #E2E8F0' }}>
                  <h3 className="text-sm font-semibold mb-3 text-[#0A1F44]">Khuyến nghị tổng quát</h3>
                  <ul className="space-y-2">
                    {report.recommendations.map((r, i) => (
                      <li key={i} className="flex gap-2 text-sm text-[#0A1F44]"><span style={{ color: '#0A1F44' }}>{i + 1}.</span>{r}</li>
                    ))}
                  </ul>
                </div>
              )}
          ...
          {report.risk_flags.length > 0 && (
            <div className="space-y-4 mt-6">
              ...
              {report.risk_flags.map((flag, i) => {
                ...
                const key = `risk-${i}`;
                return (
                  <div key={i} className="rounded-xl p-5" style={{ background: '#F5F1E8', border: `1px solid ${sev.color}30` }}>
                    ...
                    <blockquote className="rounded-lg px-4 py-3 mb-3 text-sm italic border-l-2"
                      style={{ background: `${sev.color}12`, borderColor: sev.color, color: '#0A1F44' }}>
                      "{flag.text_snippet}"
                    </blockquote>
        ...
        <div className="space-y-4">
          {report.citations.length === 0 ? (
            ...
            report.citations.map((cit, i) => (
              <div key={i} className="rounded-xl p-5" style={{ background: '#F5F1E8', border: '1px solid #E2E8F0' }}>
                ...
                <blockquote className="rounded-lg px-4 py-3 mb-3 text-sm border-l-2"
                  style={{ background: 'rgba(10,31,68,0.06)', borderColor: '#0A1F44', color: '#374151', fontStyle: 'italic' }}>
                  "{cit.text}"
                </blockquote>
                <p className="text-xs" style={{ color: '#6B7280' }}>
                  <span style={{ color: '#0A1F44' }}>Áp dụng:</span> {cit.relevance}
                </p>
              </div>
            ))
          )}
      ...
      {activeTab === 'history' && (
        <div className="space-y-4">
          <div className="rounded-xl p-4 text-sm" style={{ background: 'rgba(10,31,68,0.06)', border: '1px solid rgba(10,31,68,0.2)', color: '#6B7280' }}>
            Lịch sử phiên bản của: <strong className="text-[#0A1F44]">{fileName}</strong>
          </div>

          {/* Score progression */}
          {fileVersions.length > 1 && (
            <div className="rounded-xl p-5" style={{ background: 'rgba(10,31,68,0.06)', border: '1px solid rgba(10,31,68,0.15)' }}>
              <h3 className="text-sm font-semibold mb-3" style={{ color: '#0A1F44' }}>Tiến độ cải thiện</h3>
              ...
              {fileVersions.map((v, i) => {
                ...
                return (
                  <div key={i} className="px-4 py-3"
                    style={{ borderBottom: i < fileVersions.length - 1 ? '1px solid #E2E8F0' : 'none', background: isCurrent ? 'rgba(34,197,94,0.04)' : 'transparent' }}>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm text-[#0A1F44] flex items-center gap-2">
                          Phiên bản {v.version}
                          {isCurrent && <span className="text-xs px-1.5 py-0.5 rounded-full" style={{ background: 'rgba(10,31,68,0.06)', color: '#0A1F44' }}>mới nhất</span>}
                        </p>
      ...
      {activeTab === 'auditor' && (
        <div className="space-y-5">
          ...
          <div className="rounded-2xl p-5"
            ...
            <div className="grid items-center gap-4" style={{ gridTemplateColumns: 'auto 1fr auto' }}>
              <div className="w-12 h-12 rounded-xl grid place-items-center"
                style={{ background: 'rgba(10,31,68,0.15)', border: '1px solid rgba(10,31,68,0.3)' }}>
              ...
              <div>
                <h3 className="text-base font-bold text-[#0A1F44]">Review kiểm toán viên</h3>
              ...
              {existingReview && (
                <span className="px-3 py-1 rounded-full text-xs font-medium"
                  style={{
                    ...
                    color: existingReview.status === 'approved' ? '#0A1F44' : existingReview.status === 'rejected' ? '#f87171' : '#fbbf24',
                  }}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-xl p-4 text-center" style={{ background: '#F5F1E8', border: '1px solid #E2E8F0' }}>
            ...
            <div className="rounded-xl p-4 text-center" style={{ background: '#F5F1E8', border: '1px solid rgba(10,31,68,0.3)' }}>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {([
                  ...
                  ['approved', '✓ Duyệt', 'rgba(34,197,94,0.15)', '#0A1F44', 'rgba(10,31,68,0.25)'],
                ...
                {report.issues.map((issue, i) => {
                  ...
                  return (
                    <div key={i} className="rounded-xl overflow-hidden" style={{ border: '1px solid #E2E8F0' }}>
                      <div className="px-4 py-3" style={{ background: '#F5F1E8' }}>
                        <div className="grid items-start gap-3" style={{ gridTemplateColumns: '1fr auto' }}>
                          <div>
                            ...
                            <p className="text-sm text-[#0A1F44]">{issue.issue}</p>
                            {issue.recommendation && (
                              <p className="text-sm mt-1" style={{ color: '#0A1F44' }}>→ {issue.recommendation}</p>
                            )}
                          </div>
                          ...
                          <div className="grid grid-flow-col gap-1.5 flex-shrink-0">
                            <button onClick={() => setReview(r => ({
                              ...
                              title="Xác nhận đúng"
                              style={{
                                background: isConfirmed ? 'rgba(10,31,68,0.15)' : '#F5F1E8',
                                color: isConfirmed ? '#0A1F44' : '#6B7280',
                                border: isConfirmed ? '1px solid rgba(10,31,68,0.3)' : '1px solid #E2E8F0',
                              }}>✓</button>
                            <button onClick={() => setReview(r => ({
                              ...r,
                              ...
                              style={{
                                background: isFP ? 'rgba(239,68,68,0.2)' : '#F5F1E8',
          ...
          <div className="grid items-center gap-4 rounded-xl p-4"
            style={{ gridTemplateColumns: 'auto 1fr auto', background: '#F5F1E8', border: '1px solid #E2E8F0' }}>
            ...
            <div>
              {reviewSaved && <span className="text-sm font-medium" style={{ color: '#0A1F44' }}>✓ Đã lưu thành công</span>}
            </div>
      ...
      {activeTab === 'export' && (
        <div className="space-y-4">
          <div className="rounded-xl p-6" style={{ background: '#F5F1E8', border: '1px solid #E2E8F0' }}>
            <h3 className="text-base font-semibold mb-2 text-[#0A1F44]">Xuất báo cáo đánh giá</h3>
            ...
            <div className="grid grid-flow-col gap-3 justify-start">
              <button onClick={exportReport}
                ...
                style={{ gridTemplateColumns: 'auto 1fr', background: '#0A1F44', color: 'white' }}>
              ...
              <button onClick={() => {
                ...
                className="px-5 py-2.5 text-sm font-medium rounded-xl transition-all grid items-center gap-2"
                style={{ gridTemplateColumns: 'auto 1fr', background: 'rgba(10,31,68,0.1)', color: '#6B7280', border: '1px solid rgba(10,31,68,0.25)' }}>
                <span>{ }</span> Xuất JSON
              </button>
            </div>
          </div>
          ...
          <div className="rounded-xl p-5 text-sm" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', fontFamily: 'monospace', color: '#6B7280' }}>
            <p className="text-[#0A1F44] font-bold mb-3">BÁO CÁO ĐÁNH GIÁ HALAL COMPLIANCE</p>
            ...
            <p>Vấn đề: {report.issues.length} ({report.issues.filter(i => i.severity === 'critical').length} nghiêm trọng)</p>
...
function generateReportHTML(report: EvaluationReport, review: AuditorReview | null): string {
  const st = report.overall_status === 'compliant' ? '#0A1F44' : report.overall_status === 'needs_review' ? '#f59e0b' : '#ef4444';
