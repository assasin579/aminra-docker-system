...
export default function AuditorsPage() {
  ...
  useEffect(() => {
    if (isAuthenticated) fetchAuditors();
  }, [isAuthenticated, fetchAuditors]);
  ...
  const handleRemove = async (id: string) => {
    ...
    setRemoveId(id);
    try {
      await fetch(`/api/auth/provider/auditors/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchAuditors();
  ...
  const openCerts = async (auditorId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/auth/provider/auditors/${auditorId}/certificates`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const viewCert = (auditorId: string, certId: string) => {
    openAuthed(
      `/api/auth/provider/auditors/${auditorId}/certificates/${certId}/view`,
      token || "",
    );
  };
  ...
  const deleteCert = async (auditorId: string, certId: string) => {
    ...
    await fetch(
      `/api/auth/provider/auditors/${auditorId}/certificates/${certId}`,
      {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      },
          ...
          <div className="grid grid-flow-col items-center gap-3 justify-start">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: "#6B7280" }}>
                {auditors.length} auditor · {user.company_name}
              </p>
            </div>
          </div>
          <button
      ...
      <div className="flex-1 lg:min-h-0 lg:overflow-y-auto space-y-3">
        {fetching ? (
          ...
          >
            ...
            <p className="text-sm" style={{ color: "#6B7280" }}>
              Tạo auditor để gán đánh giá hồ sơ doanh nghiệp
            </p>
          </div>
        ) : (
          auditors.map((a, idx) => (
            ...
            <form onSubmit={handleInvite} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setInvite((i) => ({ ...i, display_name: e.target.value }))
                  }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setInvite((i) => ({ ...i, specialty: e.target.value }))
                  }
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (f && certsOpen) uploadCert(certsOpen, f);
                  }}
                />
                <button
              ...
              {certsLoading ? (
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="text-xs mt-1" style={{ color: "#9CA3AF" }}>
                    Upload bằng đại học, chứng chỉ đào tạo, giấy phép hành
                    nghề,...
                  </p>
                </div>
              ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs" style={{ color: "#9CA3AF" }}>
                        ...
                        {new Date(c.uploaded_at + "Z").toLocaleDateString(
                          "vi-VN",
                        )}
                      </p>
                    </div>
