...
export default function PortfolioPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setFetching(true);
    try {
      const res = await fetch("/api/api/audits/businesses", {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: "#6B7280" }}>
                {businesses.length} doanh nghiệp đang quản lý
              </p>
            </div>
          </div>
          <input
        ...
        {fetching ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className={`rounded-xl p-5 animate-list-item stagger-${i}`}
                style={{ background: "#FFFFFF", border: "1px solid #E2E8F0" }}
              >
                <div className="flex gap-4">
                  <div className="shimmer w-10 h-10 rounded-full" />
                  <div className="flex-1 space-y-2">
                    <div className="shimmer skeleton-text w-40" />
          ...
          <div className="text-center py-12 animate-scale-in">
            ...
            <p className="font-semibold" style={{ color: "#0A1F44" }}>
              {search ? "Không tìm thấy" : "Chưa có doanh nghiệp nào"}
            </p>
            <p className="text-sm mt-1" style={{ color: "#94A3B8" }}>
              Doanh nghiệp sẽ xuất hiện khi gửi hồ sơ hoặc được kiểm định
            </p>
          </div>
        ) : (
          filtered.map((biz, idx) => {
            ...
            return (
              <div
                key={biz.id}
                className={`rounded-xl overflow-hidden doc-card-hover animate-list-item stagger-${Math.min(idx + 1, 12)}`}
                style={{ background: "#FFFFFF", border: "1px solid #E2E8F0" }}
              >
                {/* Header */}
                <button
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-1">
                        {biz.active_certs > 0 && (
                          <span
                            ...
                            style={{ background: "#DCE3F0", color: "#0A1F44" }}
                          >
                        ...
                        {biz.latest_score !== null && (
                          <span
                            ...
                            style={{ color: scoreColor(biz.latest_score) }}
                          >
                            {biz.latest_score}%
                          </span>
                        )}
                ...
                {isOpen && (
                  <div
                    className="px-5 pb-5 animate-tab-content"
                    ...
                    {biz.nearest_expiry &&
                      ...
                      expiryDays <= 90 && (
                        <div
                          ...
                          style={{
                            background:
                              expiryDays <= 30 ? "#FEF2F2" : "#FEF3C7",
                            border: `1px solid ${expiryDays <= 30 ? "#FECACA" : "#FDE68A"}`,
                          }}
                        >
                          <span
                            className="text-sm"
                            style={{
                              color: expiryDays <= 30 ? "#DC2626" : "#D97706",
                            }}
                          >
                            ⚠️ Chứng nhận hết hạn{" "}
                            {expiryDays <= 0
                              ? "đã quá hạn"
                              : `trong ${expiryDays} ngày`}{" "}
                            ({fmtDate(biz.nearest_expiry)})
                          </span>
                        </div>
                      )}
