...
export default function ProviderBusinessPage({
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [dRes, sRes] = await Promise.all([
        fetch(`/api/api/audits/businesses/${id}/dossier`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`/api/api/audits/businesses/${id}/score`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  if (loading || fetching) {
    return (
      <div
        className="max-w-5xl mx-auto px-4 py-12 text-sm"
        style={{ color: "#94A3B8" }}
      >
        Đang tải...
      </div>
    ...
    <div className="max-w-5xl mx-auto px-4 py-8" data-page>
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <Link
            href="/portfolio"
            className="inline-flex items-center min-h-[32px] text-sm font-medium"
            style={{ color: "#0A1F44" }}
          >
            ← Portfolio
          </Link>
          <h1 className="text-2xl font-bold mt-1" style={{ color: "#0A1F44" }}>
            {dossier.business.company_name}
          </h1>
          <p className="text-xs mt-1" style={{ color: "#94A3B8" }}>
            {dossier.business.email}
            {dossier.business.phone ? " • " + dossier.business.phone : ""}
          </p>
        </div>
      </div>
      ...
      >
            ...
            <div className="flex items-baseline gap-3">
              ...
              >
                ...
                <span
                  ...
                  style={{ color: "#94A3B8" }}
                >
                  {score.composite_score !== null ? "/100" : ""}
                </span>
              </span>
              <span
        ...
        <p
          ...
          style={{ color: "#64748B" }}
        >
          Điểm tổng = 40% điểm tài liệu (lấy revision mới nhất mỗi loại) + 60%
          điểm kiểm định thực tế (visit đã hoàn thành). Khi chỉ có 1 nguồn dữ
          liệu, dùng 100% nguồn đó.
        </p>
      </section>
      ...
      >
        ...
        >
          <h2 className="text-base font-bold" style={{ color: "#0A1F44" }}>
            Thư mục tài liệu — {dossier.documents_by_type.length} loại
          </h2>
        </div>
        {dossier.documents_by_type.length === 0 ? (
          <p
            ...
            style={{ color: "#94A3B8" }}
          >
            Chưa có tài liệu nào từ doanh nghiệp này
          </p>
        ) : (
          <div>
            {dossier.documents_by_type.map((b) => {
              ...
              return (
                ...
                >
                  <button
                    onClick={() => setOpenType(isOpen ? null : b.doc_type)}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p
                          ...
                          style={{ color: "#64748B" }}
                        >
                          {b.revision_count} revision • Mới nhất:{" "}
                          {b.current?.original_filename ?? "—"}
                          {b.current?.compliance_score !== null &&
                            b.current?.compliance_score !== undefined && (
                              <span
                                className="ml-2 font-bold"
                                style={{ color: "#0A1F44" }}
                              >
                                {b.current.compliance_score}/100
                              </span>
                            )}
                        </p>
                      </div>
      ...
      >
        ...
        >
          <h2 className="text-base font-bold" style={{ color: "#0A1F44" }}>
            Hồ sơ đã gửi — {dossier.submissions.length}
          </h2>
        </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm" style={{ color: "#0A1F44" }}>
                    {s.document_ids.length} tài liệu • Gửi{" "}
      ...
      >
        ...
        >
          <h2 className="text-base font-bold" style={{ color: "#0A1F44" }}>
            Kiểm định thực tế — {dossier.audit_visits.length}
          </h2>
        </div>
          ...
          <div>
            {dossier.audit_visits.map((v) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm" style={{ color: "#0A1F44" }}>
                    {v.visit_type} • {fmtDate(v.scheduled_date)}
                  </p>
                </div>
                {v.compliance_score !== null && (
                  <span
                    ...
                    style={{
                      color:
                        v.compliance_score >= 80
                          ? "#0A1F44"
                          : v.compliance_score >= 60
                            ? "#D97706"
                            : "#DC2626",
                    }}
                  >
  ...
  return (
    <div className="text-right">
      ...
      >
        {score ?? "—"}
        {score !== null && (
          <span className="text-xs font-medium opacity-60 ml-1">/100</span>
        )}
      </p>
