  ...
  return (
    <div data-page className="flex flex-col flex-1 lg:min-h-0 w-full overflow-x-hidden">
      ...
      <div className="rounded-2xl p-6 mb-6 animate-section"
        ...
        <div className="grid items-center" style={{ gridTemplateColumns: '1fr auto' }}>
          <div className="grid grid-flow-col items-center gap-3 justify-start">
            ...
            <div>
              <h1 className="text-xl font-bold" style={{ color: '#0F5132' }}>Internal Halal Committee</h1>
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            ...
            {tab.id === 'minutes' && minutes.length > 0 && (
              <span className="px-1.5 py-0.5 rounded text-xs animate-chip"
                style={{ background: activeTab === tab.id ? 'rgba(255,255,255,0.2)' : 'rgba(15,81,50,0.15)', color: activeTab === tab.id ? '#fff' : '#0F5132' }}>
                {minutes.length}
              </span>
            )}
          </button>
      ...
      <div className="rounded-xl p-4 mb-4 doc-card-hover animate-section"
        style={{ background: '#FFFFFF', border: '1px solid rgba(15,81,50,0.3)' }}>
        <div className="grid items-center gap-3" style={{ gridTemplateColumns: 'auto 1fr auto' }}>
          <div className="w-10 h-10 rounded-full grid place-items-center text-sm font-bold"
            style={{ background: 'rgba(15,81,50,0.15)', color: '#0F5132', border: '1px solid rgba(15,81,50,0.3)' }}>
            {user.company_name?.[0]?.toUpperCase() || 'O'}
          </div>
          <div>
            <p className="text-sm font-semibold" style={{ color: '#0F5132' }}>{user.email}</p>
            <p className="text-sm" style={{ color: '#6B7280' }}>Chủ tài khoản</p>
          </div>
          <span className="px-3 py-1 rounded-lg text-xs font-bold"
            style={{ background: 'rgba(15,81,50,0.12)', color: '#0F5132', border: '1px solid rgba(15,81,50,0.25)' }}>
            Owner
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 lg:min-h-0 lg:overflow-y-auto space-y-3">
        {fetching ? (
          <div className="space-y-3">
            {[1, 2, 3].map(i => (
              <div key={i} className="rounded-2xl h-20 shimmer" style={{ background: '#F7F1E6', opacity: 1 - i * 0.2 }} />
            ))}
          </div>
        ) : members.length === 0 ? (
          <div className="rounded-2xl p-12 text-center animate-scale-in" style={{ background: '#F7F1E6', border: '1px solid #E2E8F0' }}>
            ...
            <p className="font-semibold mb-2" style={{ color: '#0F5132' }}>Chưa có thành viên IHC</p>
                  ...
                  <div className="grid items-center gap-3" style={{ gridTemplateColumns: 'auto 1fr auto' }}>
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-sm font-semibold" style={{ color: '#0F5132' }}>{m.display_name}</p>
                        {m.ihc_role && (
        ...
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-modal-overlay"
          ...
          <div className="w-full max-w-md rounded-2xl p-6 animate-modal-content"
            ...
            <div className="grid items-center mb-5" style={{ gridTemplateColumns: '1fr auto' }}>
              <h3 className="text-base font-bold" style={{ color: '#0F5132' }}>Mời thành viên IHC</h3>
          ...
          <div className="flex-1 lg:min-h-0 lg:overflow-y-auto space-y-3">
            {fetchingMinutes ? (
              ...
              <div className="rounded-2xl p-12 text-center animate-scale-in" style={{ background: '#F7F1E6', border: '1px solid #E2E8F0' }}>
                ...
                <p className="font-semibold mb-2" style={{ color: '#0F5132' }}>Chưa có biên bản họp</p>
              ...
              minutes.map((m, i) => (
                <div key={m.id} className={`rounded-xl p-4 transition-colors doc-card-hover animate-list-item stagger-${Math.min(i + 1, 12)}`}
                  ...
                  <div className="grid items-center gap-3" style={{ gridTemplateColumns: 'auto 1fr auto' }}>
                    ...
                    {/* Info */}
                    <div className="min-w-0">
                      <p className="text-sm font-semibold truncate" style={{ color: '#0F5132' }} title={m.original_filename}>
                        {m.original_filename}
                      </p>
                      <p className="text-sm mt-0.5" style={{ color: '#6B7280' }}>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-modal-overlay"
          ...
          <div className="w-full max-w-sm rounded-2xl p-6 animate-modal-content"
            ...
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-base font-bold" style={{ color: '#0F5132' }}>Phân quyền thành viên</h3>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={p.key} className="flex items-center justify-between gap-3 px-4 py-3 rounded-xl"
                  style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
                  <div>
                    <p className="text-sm font-medium" style={{ color: '#0F5132' }}>{p.label}</p>
                  ...
                  <button onClick={() => setPermData(d => ({ ...d, [p.key]: !d[p.key] }))}
                    ...
                    style={{ background: permData[p.key] ? '#0F5132' : '#E2E8F0' }}>
      ...
      {showInviteLink && (
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-modal-overlay"
          ...
          <div className="w-full max-w-md rounded-2xl p-6 animate-modal-content"
            ...
            onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-base font-bold" style={{ color: '#0F5132' }}>Mời thành viên qua link</h3>
              <button onClick={() => setShowInviteLink(false)} style={{ color: '#6B7280' }}>✕</button>
            </div>

            {generatedLink ? (
              <div className="space-y-4">
                <div className="rounded-lg p-4" style={{ background: '#E8F5EF', border: '1px solid #B7CBB8' }}>
                  <p className="text-sm font-medium mb-2" style={{ color: '#198754' }}>Link mời đã tạo!</p>
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(generatedLink); alert('Đã copy!'); }}
                      className="px-3 py-2 rounded-lg text-xs font-medium text-white flex-shrink-0" style={{ background: '#0F5132' }}>
                      Copy
                    </button>
                  </div>
