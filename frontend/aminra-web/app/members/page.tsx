...
export default function MembersPage() {
  ...
  const fetchMembers = useCallback(async () => {
    ...
    setFetching(true);
    try {
      const res = await fetch("/api/auth/business/members", {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(parseApiError(err, "Mời thất bại"));
      }
      setShowInvite(false);
      setInvite({
        email: "",
  ...
  const handleRemove = async (id: string) => {
    ...
    setRemoveId(id);
    try {
      await fetch(`/api/auth/business/members/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchMembers();
  ...
  const handleExportIHC = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = cd.match(/filename="([^"]+)"/)?.[1] || "IHC_Document.docx";
      a.click();
  ...
  useEffect(() => {
    if (isAuthenticated && activeTab === "minutes") fetchMinutes();
  }, [isAuthenticated, activeTab, fetchMinutes]);
  ...
  const deleteMinutes = async (fileId: string) => {
    ...
    try {
      await fetch(`/api/auth/business/minutes/${fileId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchMinutes();
  ...
  return (
    ...
    >
        ...
        {[
          ...
          >
            ...
            {tab.id === "minutes" && minutes.length > 0 && (
              <span
                className="px-1.5 py-0.5 rounded text-xs animate-chip"
                style={{
                  background:
                    activeTab === tab.id
                      ? "rgba(255,255,255,0.2)"
                      : "rgba(10,31,68,0.15)",
                  color: activeTab === tab.id ? "#fff" : "#0A1F44",
                }}
              >
                {minutes.length}
              </span>
      ...
      {/* ── Tab: Members ── */}
      {activeTab === "members" && (
        <div key="members" className="animate-tab-content">
          {/* Owner card */}
          <div
            className="rounded-xl p-4 mb-4 doc-card-hover animate-section"
            style={{
              background: "#FFFFFF",
            ...
            {fetching ? (
              ...
              >
                ...
                <p className="font-semibold mb-2" style={{ color: "#0A1F44" }}>
                  Chưa có thành viên IHC
                </p>
                <p className="text-sm mb-4" style={{ color: "#6B7280" }}>
                  Mời thành viên và phân vai trò trong Internal Halal Committee
                </p>
              </div>
            ) : (
              members.map((m, i) => {
                ...
                return (
                  <div
                    key={m.id}
                    className={`rounded-xl p-4 transition-colors doc-card-hover animate-list-item stagger-${Math.min(i + 1, 12)}`}
                    style={{
                      background: "#FFFFFF",
                      ...
                      <div className="space-y-3">
                        <div className="grid grid-cols-3 gap-3">
                          ...
                          <div>
                            ...
                            <input
                              value={editData.ihc_role}
                              onChange={(e) =>
                                setEditData((d) => ({
                                  ...d,
                                  ihc_role: e.target.value,
                                }))
                              }
                              list="ihc-roles"
                          ...
                          <button
                            onClick={async () => {
                              ...
                              const res = await fetch(
                                `/api/auth/business/members/${m.id}/permissions`,
                                {
                                  headers: { Authorization: `Bearer ${token}` },
                                },
                              );
                ...
                <form onSubmit={handleInvite} className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        value={invite.ihc_role}
                        onChange={(e) =>
                          setInvite((i) => ({ ...i, ihc_role: e.target.value }))
                        }
                        list="invite-roles"
                        placeholder="Chairman, Executive..."
                        className="w-full px-4 py-2.5 rounded-xl text-sm outline-none"
                        style={{
                          background: "#FFFFFF",
                          border: "1px solid #E2E8F0",
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setInvite((i) => ({ ...i, password: e.target.value }))
                      }
                  ...
                  <p
                    ...
                    style={{ color: "#94A3B8" }}
                  >
                    Thành viên đăng nhập bằng email + mật khẩu trên. Chỉ có
                    quyền xem, không thể chỉnh sửa.
                  </p>
                </form>
              </div>
            </Modal>
      ...
      {activeTab === "minutes" && (
        <div
          key="minutes"
          className="flex-1 lg:min-h-0 flex flex-col animate-tab-content"
        >
          {/* Upload bar */}
          <div
            className="grid items-center gap-3 mb-4 animate-section"
            style={{ gridTemplateColumns: "1fr auto" }}
          >
            <p className="text-sm" style={{ color: "#6B7280" }}>
              {minutes.length} biên bản họp
            </p>
            <div>
              <input
                ref={minutesInputRef}
          ...
          <div className="flex-1 lg:min-h-0 lg:overflow-y-auto space-y-3">
            {fetchingMinutes ? (
              ...
              >
                ...
                <p className="font-semibold mb-2" style={{ color: "#0A1F44" }}>
                  Chưa có biên bản họp
                </p>
                <p className="text-sm" style={{ color: "#6B7280" }}>
                  Upload biên bản họp IHC để lưu trữ và theo dõi
                </p>
              </div>
            ) : (
              minutes.map((m, i) => (
                ...
                >
                  ...
                  >
                    ...
                    <div
                      className="w-10 h-10 rounded-xl grid place-items-center"
                      style={{
                        background: m.mime_type.includes("pdf")
                          ? "rgba(239,68,68,0.12)"
                    ...
                    >
                      <span
                        ...
                        style={{
                          color: m.mime_type.includes("pdf")
                            ? "#f87171"
                            : "#64748b",
                        }}
                      >
                    ...
                    <div className="min-w-0">
                      ...
                      >
                        ...
                        {new Date(m.uploaded_at).toLocaleString("vi-VN", {
                          day: "2-digit",
                          month: "2-digit",
                          year: "numeric",
                          hour: "2-digit",
        ...
        <Modal onClose={() => setPermModal(null)}>
          ...
          >
            ...
            <p className="text-xs mb-4" style={{ color: "#6B7280" }}>
              {members.find((m) => m.id === permModal)?.display_name ||
                "Thành viên"}
            </p>
            <div className="space-y-3">
              {[
                {
                  ...
                  desc: "Sửa tài liệu, nguyên liệu, NCC, quy trình",
                },
                {
                  key: "can_delete",
                  label: "Xóa dữ liệu",
                  desc: "Xóa tài liệu, nguyên liệu, NCC",
                },
                {
                  key: "can_approve",
                  label: "Xác nhận / Phê duyệt",
                  desc: "Xác nhận bước sản xuất, seal lô hàng",
                },
                {
                  key: "can_upload",
                  label: "Upload tài liệu",
                  desc: "Upload tài liệu mới, ảnh minh chứng",
                },
              ].map((p) => (
                <div
                  key={p.key}
            ...
            <button
              onClick={async () => {
                ...
                try {
                  await fetch(
                    `/api/auth/business/members/${permModal}/permissions`,
                    {
                      method: "PUT",
                      headers: {
                        "Content-Type": "application/json",
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <p className="text-xs mt-2" style={{ color: "#6B7280" }}>
                    Gửi link này cho thành viên. Họ sẽ tự tạo mật khẩu khi tham
                    gia.
                  </p>
                </div>
                <button
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setInviteLink((f) => ({ ...f, email: e.target.value }))
                    }
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const d = await res.json();
                      setGeneratedLink(
                        `${window.location.origin}${d.portal_url}`,
                      );
                    } finally {
                  ...
                  style={{
                    background: !inviteLink.email ? "#E2E8F0" : "#7C3AED",
                    color: !inviteLink.email ? "#9CA3AF" : "#fff",
                  }}
                >
                  {invitingLink ? "Đang tạo..." : "Tạo link mời"}
                </button>
                <p className="text-xs text-center" style={{ color: "#9CA3AF" }}>
                  Link hết hạn sau 7 ngày. Thành viên tự tạo mật khẩu khi tham
                  gia.
                </p>
              </div>
            )}
