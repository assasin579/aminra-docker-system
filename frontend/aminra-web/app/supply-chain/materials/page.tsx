...
interface Supplier {
  ...
  email: string | null; contact_person: string | null; supplier_type: string | null;
  status: string; notes: string | null; material_count: number; cert_count: number;
  created_at: string;
}

interface Certificate {
  id: string; cert_type: string | null; cert_number: string | null;
  issuing_body: string | null; issued_date: string | null; expiry_date: string | null;
  original_filename: string | null; file_size: number | null; created_at: string;
}

interface Material {
  id: string; name: string; sku: string | null; category: string | null;
  halal_risk: string; description: string | null; unit: string | null;
  supplier_id: string; supplier_name: string | null; created_at: string;
}

const CATEGORIES = [
  { id: 'meat', label: 'Thịt' }, { id: 'dairy', label: 'Sữa' },
  { id: 'grain', label: 'Ngũ cốc' }, { id: 'spice', label: 'Gia vị' },
  { id: 'additive', label: 'Phụ gia' }, { id: 'chemical', label: 'Hóa chất' },
  { id: 'packaging', label: 'Bao bì' }, { id: 'other', label: 'Khác' },
];
...
export default function MaterialsPage() {
  ...
  const [matForm, setMatForm] = useState({ name: '', sku: '', category: '', halal_risk: 'unknown', description: '', unit: '', supplier_id: '' });
  ...
  const [supForm, setSupForm] = useState({ name: '', address: '', phone: '', email: '', contact_person: '', supplier_type: '', tax_code: '', notes: '' });
  ...
  const fetchMaterials = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/api/supply-chain/materials?${params}`, { headers });
      if (res.ok) { const d = await res.json(); setMaterials(d.materials || []); }
  ...
  useEffect(() => { if (isAuthenticated) { fetchSuppliers(); fetchMaterials(); } }, [isAuthenticated, fetchSuppliers, fetchMaterials]);

  // ── Material CRUD ──────────────────────────────────────────────────────────

  const openMatCreate = () => {
    setEditMat(null);
    setMatForm({ name: '', sku: '', category: '', halal_risk: 'unknown', description: '', unit: '', supplier_id: '' });
    setShowMatForm(true);
  };
  const openMatEdit = (m: Material) => {
    setEditMat(m);
    setMatForm({ name: m.name, sku: m.sku || '', category: m.category || '', halal_risk: m.halal_risk, description: m.description || '', unit: m.unit || '', supplier_id: m.supplier_id });
    setShowMatForm(true);
  };
  const saveMaterial = async () => {
    ...
    setMatSaving(true);
    try {
      const url = editMat ? `/api/api/supply-chain/materials/${editMat.id}` : '/api/api/supply-chain/materials';
  ...
  const deleteMaterial = async (id: string) => {
    ...
    await fetch(`/api/api/supply-chain/materials/${id}`, { method: 'DELETE', headers });
    fetchMaterials();
  };
  ...
  const openSupCreate = () => {
    setEditSup(null);
    setSupForm({ name: '', address: '', phone: '', email: '', contact_person: '', supplier_type: '', tax_code: '', notes: '' });
    setShowSupForm(true);
  };
  const openSupEdit = (s: Supplier) => {
    setEditSup(s);
    setSupForm({ name: s.name, address: s.address || '', phone: s.phone || '', email: s.email || '', contact_person: s.contact_person || '', supplier_type: s.supplier_type || '', tax_code: (s as any).tax_code || '', notes: s.notes || '' });
    setShowSupForm(true);
  };
  // saveSupplier is now inline in the form button
  const deleteSupplier = async (id: string) => {
    if (!confirm('Xác nhận xoá nhà cung cấp? Nguyên liệu liên kết sẽ bị ảnh hưởng.')) return;
    await fetch(`/api/api/supply-chain/suppliers/${id}`, { method: 'DELETE', headers });
    fetchSuppliers();
    fetchMaterials();
  };
  ...
  const openCerts = async (supId: string) => {
    setCertsOpen(supId); setCertsLoading(true);
    try {
      const res = await fetch(`/api/api/supply-chain/suppliers/${supId}/certificates`, { headers });
  ...
  const uploadCert = async (supId: string, file: File) => {
    ...
    try {
      const fd = new FormData(); fd.append('file', file); fd.append('cert_type', certType);
      const res = await fetch(`/api/api/supply-chain/suppliers/${supId}/certificates`, { method: 'POST', headers, body: fd });
  ...
  const deleteCert = async (supId: string, certId: string) => {
    ...
    await fetch(`/api/api/supply-chain/suppliers/${supId}/certificates/${certId}`, { method: 'DELETE', headers });
    openCerts(supId);
    fetchSuppliers();
  };
  ...
  const sendInvite = async (supId: string) => {
    ...
    try {
      const res = await fetch(`/api/api/supply-chain/suppliers/${supId}/invite`, { method: 'POST', headers });
      if (!res.ok) { alert('Lỗi tạo link'); return; }
  ...
  const verifySupplier = async (supId: string) => {
    const res = await fetch(`/api/api/supply-chain/suppliers/${supId}/verify`, { method: 'POST', headers });
    if (!res.ok) { const e = await res.json().catch(() => ({})); alert(e.detail || 'Không thể xác minh'); return; }
  ...
  return (
    <div data-page className="flex flex-col flex-1 lg:min-h-0 w-full overflow-x-hidden">
      ...
      {/* Header */}
      <div className="rounded-2xl p-6 mb-5 animate-section" style={{ background: '#F5F1E8', border: '1px solid #E2E8F0' }}>
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold" style={{ color: '#0A1F44' }}>Nguyên vật liệu đầu vào</h1>
      ...
      {/* Sub-tabs */}
      <div className="grid grid-cols-2 gap-1 mb-5 p-1 rounded-xl animate-section" style={{ background: '#F5F1E8', border: '1px solid #E2E8F0' }}>
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            ...
            <span className="px-1.5 py-0.5 rounded text-xs"
              style={{ background: activeTab === tab.id ? 'rgba(255,255,255,0.2)' : '#E2E8F0' }}>
              {tab.count}
            </span>
          </button>
        ))}
      ...
      {/* ── Tab: Materials ── */}
      {activeTab === 'materials' && (
        <div key="materials" className="flex-1 lg:min-h-0 lg:overflow-y-auto space-y-3 animate-tab-content">
          {/* Search + filter */}
          <div className="flex gap-2 flex-wrap mb-3">
            <input type="text" placeholder="Tìm theo tên hoặc SKU..." value={matSearch}
              onChange={e => setMatSearch(e.target.value)}
              className="flex-1 min-w-[200px] px-4 py-2.5 rounded-xl text-sm outline-none" style={inputStyle} />
          ...
          {matLoading ? (
            ...
            <div className="rounded-2xl p-12 text-center animate-scale-in" style={cardStyle}>
              <p className="font-semibold" style={{ color: '#0A1F44' }}>Chưa có nguyên liệu nào</p>
            ...
            materials.map((m, idx) => {
              ...
              return (
                <div key={m.id} className={`rounded-xl p-4 doc-card-hover animate-list-item stagger-${Math.min(idx + 1, 12)}`} style={cardStyle}>
                  <div className="flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <span className="text-sm font-semibold" style={{ color: '#0A1F44' }}>{m.name}</span>
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => openMatEdit(m)} title="Sửa"
                      ...
                      <button onClick={() => deleteMaterial(m.id)} title="Xoá"
                        className="w-8 h-8 rounded-lg grid place-items-center transition-all hover:scale-110"
      ...
      {activeTab === 'suppliers' && (
        <div key="suppliers" className="flex-1 lg:min-h-0 lg:overflow-y-auto space-y-3 animate-tab-content">
          {supLoading ? (
            ...
            <div className="rounded-2xl p-12 text-center animate-scale-in" style={cardStyle}>
              <p className="font-semibold" style={{ color: '#0A1F44' }}>Chưa có nhà cung cấp nào</p>
              ...
              return (
                <div key={s.id} className={`rounded-xl p-4 doc-card-hover animate-list-item stagger-${Math.min(idx + 1, 12)}`} style={cardStyle}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <span className="text-sm font-semibold" style={{ color: '#0A1F44' }}>{s.name}</span>
                      ...
                      <p className="text-xs" style={{ color: '#6B7280' }}>
                        {(s as any).tax_code && `MST: ${(s as any).tax_code} · `}{s.material_count} nguyên liệu · {s.cert_count} chứng chỉ
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {s.status !== 'verified' && s.cert_count > 0 && (
                        <button onClick={() => verifySupplier(s.id)} title="Xác minh NCC"
                          ...
                          <svg className="w-4 h-4" style={{ color: '#102A5C' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                          </svg>
                        </button>
                      )}
                      {/* Invite NCC */}
                      <button onClick={() => sendInvite(s.id)} title="Gửi yêu cầu hồ sơ"
                      ...
                      <button onClick={() => openCerts(s.id)} title="Chứng chỉ"
                        className="w-8 h-8 rounded-lg grid place-items-center transition-all hover:scale-110"
                      ...
                      <button onClick={() => openSupEdit(s)} title="Sửa"
                        className="w-8 h-8 rounded-lg grid place-items-center transition-all hover:scale-110"
                        style={{ background: '#F0F9FF', border: '1px solid #BAE6FD' }}>
                        <svg className="w-4 h-4" style={{ color: '#0369A1' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                      ...
                      <button onClick={() => deleteSupplier(s.id)} title="Xoá"
                        className="w-8 h-8 rounded-lg grid place-items-center transition-all hover:scale-110"
                        style={{ background: '#FEF2F2', border: '1px solid #FECACA' }}>
                        <svg className="w-4 h-4" style={{ color: '#DC2626' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
        ...
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-modal-overlay"
          ...
          <div className="w-full max-w-md rounded-2xl p-6 animate-modal-content"
            ...
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-base font-bold" style={{ color: '#0A1F44' }}>{editMat ? 'Sửa nguyên liệu' : 'Thêm nguyên liệu'}</h3>
              <button onClick={() => setShowMatForm(false)} style={{ color: '#6B7280' }}>✕</button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Tên nguyên liệu *</label>
                <input value={matForm.name} onChange={e => setMatForm(f => ({ ...f, name: e.target.value }))}
                  className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle} placeholder="VD: Bột mì số 11" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Mã SKU</label>
                  <input value={matForm.sku} onChange={e => setMatForm(f => ({ ...f, sku: e.target.value }))}
                    className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle} placeholder="VD: NL-001" />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Đơn vị</label>
                  <input value={matForm.unit} onChange={e => setMatForm(f => ({ ...f, unit: e.target.value }))}
                    className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle} placeholder="kg, lít, cái" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Phân loại</label>
                ...
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Rủi ro Halal</label>
                  <select value={matForm.halal_risk} onChange={e => setMatForm(f => ({ ...f, halal_risk: e.target.value }))}
                    className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle}>
              ...
              <div>
                <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Nhà cung cấp *</label>
                <select value={matForm.supplier_id} onChange={e => setMatForm(f => ({ ...f, supplier_id: e.target.value }))}
                  className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle}>
              ...
              <button onClick={saveMaterial} disabled={matSaving || !matForm.name || !matForm.supplier_id}
                ...
                style={{ background: (!matForm.name || !matForm.supplier_id) ? '#E2E8F0' : '#0A1F44',
      ...
      {showSupForm && (
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-modal-overlay"
          ...
          <div className="w-full max-w-md rounded-2xl p-6 animate-modal-content"
            ...
            onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-base font-bold" style={{ color: '#0A1F44' }}>{editSup ? 'Sửa nhà cung cấp' : 'Thêm nhà cung cấp'}</h3>
              <button onClick={() => setShowSupForm(false)} style={{ color: '#6B7280' }}>✕</button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Tên nhà cung cấp *</label>
                <input value={supForm.name} onChange={e => setSupForm(f => ({ ...f, name: e.target.value }))}
                  className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle} placeholder="VD: Công ty TNHH Nguyên liệu ABC" />
              </div>
              <div>
                <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Mã số thuế</label>
                <input value={supForm.tax_code} onChange={e => setSupForm(f => ({ ...f, tax_code: e.target.value }))}
                  className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle} placeholder="VD: 0312345678" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Điện thoại</label>
                ...
                <div>
                  ...
                  <input value={supForm.email} onChange={e => setSupForm(f => ({ ...f, email: e.target.value }))}
                    className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle} />
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Địa chỉ</label>
                <input value={supForm.address} onChange={e => setSupForm(f => ({ ...f, address: e.target.value }))}
                  className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Người liên hệ</label>
                  <input value={supForm.contact_person} onChange={e => setSupForm(f => ({ ...f, contact_person: e.target.value }))}
                    className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle} />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Loại NCC</label>
                  <input value={supForm.supplier_type} onChange={e => setSupForm(f => ({ ...f, supplier_type: e.target.value }))}
                    className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle} placeholder="VD: Thịt, Gia vị..." />
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Ghi chú</label>
                <textarea value={supForm.notes} onChange={e => setSupForm(f => ({ ...f, notes: e.target.value }))}
                  rows={2} className="w-full px-4 py-2.5 rounded-lg text-sm outline-none resize-none" style={inputStyle} />
              </div>
              ...
              <button onClick={async () => {
                  ...
                  setSupSaving(true);
                  try {
                    const url = editSup ? `/api/api/supply-chain/suppliers/${editSup.id}` : '/api/api/supply-chain/suppliers';
                    const res = await fetch(url, {
                ...
                className="w-full py-2.5 rounded-xl text-sm font-semibold text-white"
                style={{ background: !supForm.name ? '#E2E8F0' : '#0A1F44', color: !supForm.name ? '#9CA3AF' : '#fff' }}>
      ...
      {certsOpen && (
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-modal-overlay"
          ...
          <div className="w-full max-w-lg rounded-2xl animate-modal-content"
            style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', maxHeight: 'calc(100vh - 4rem)' }}
            onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid #E2E8F0' }}>
              <div>
                <h3 className="text-base font-bold" style={{ color: '#0A1F44' }}>Hồ sơ & Chứng chỉ</h3>
                <p className="text-xs mt-0.5" style={{ color: '#6B7280' }}>
                  {suppliers.find(s => s.id === certsOpen)?.name} · Được gửi bởi NCC
                </p>
              </div>
              <button onClick={() => setCertsOpen(null)} style={{ color: '#6B7280' }}>✕</button>
            </div>
            <div className="px-6 py-4 space-y-2 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 12rem)' }}>
              {certsLoading ? (
                ...
                <div className="py-12 text-center animate-scale-in">
                  <p className="text-sm font-medium" style={{ color: '#374151' }}>NCC chưa gửi hồ sơ</p>
                  <p className="text-xs mt-1" style={{ color: '#9CA3AF' }}>Gửi yêu cầu hồ sơ để NCC tự upload chứng chỉ qua portal</p>
                </div>
              ) : (
                certs.map((c, idx) => (
                  <div key={c.id} className={`flex items-center gap-3 px-4 py-3 rounded-xl group animate-list-item stagger-${Math.min(idx + 1, 12)}`}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate" style={{ color: '#0A1F44' }}>{c.original_filename}</p>
