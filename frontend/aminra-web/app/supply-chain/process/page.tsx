...
interface ProcessTemplate {
  ...
  version: number; is_active: boolean; created_at: string; updated_at: string;
}

interface FlowNode {
  id: string; type: 'main' | 'sub'; order?: number;
  label: string; description?: string; standard?: string;
  responsible?: string; duration?: string; equipment?: string;
  conditions?: string; checklist?: string[]; notes?: string;
  x: number; y: number; parent?: string; children?: string[];
}

interface FlowEdge {
  from: string; to: string; type: 'sequential' | 'parallel';
}

// ── Custom Node ──────────────────────────────────────────────────────────────

function StepNode({ data, selected }: { data: { label: string; nodeType: string; order?: number }; selected: boolean }) {
  const isMain = data.nodeType === 'main';
  return (
    <div className="rounded-xl px-4 py-3 min-w-[160px] transition-shadow"
      style={{
        background: isMain ? '#FFFFFF' : '#FAFCF9',
        border: `2px solid ${selected ? '#087653' : isMain ? '#E2E8F0' : '#F0F7F4'}`,
        boxShadow: selected ? '0 0 0 3px rgba(8,118,83,0.15)' : '0 1px 4px rgba(0,0,0,0.06)',
      }}>
      <Handle type="target" position={Position.Top} style={{ background: '#087653', width: 8, height: 8 }} />
      <div className="flex items-center gap-2">
        {data.order != null && (
          <span className="w-6 h-6 rounded-full grid place-items-center text-xs font-bold text-white flex-shrink-0"
            style={{ background: isMain ? '#087653' : '#9CA3AF' }}>
            {data.order}
          </span>
        )}
        <div>
          <span className="text-xs font-bold" style={{ color: '#1A2332' }}>{data.label}</span>
          <div className="text-xs mt-0.5" style={{ color: '#9CA3AF' }}>
            {isMain ? 'Bước chính' : 'Bước phụ'}
          </div>
        </div>
      </div>
      <Handle type="source" position={Position.Bottom} style={{ background: '#087653', width: 8, height: 8 }} />
    </div>
  );
}
...
function flowToReact(nodes: FlowNode[], edges: FlowEdge[]): { rfNodes: Node[]; rfEdges: Edge[] } {
  ...
  const rfEdges: Edge[] = edges.map((e, i) => ({
    ...
    style: { stroke: e.type === 'parallel' ? '#B45309' : '#087653', strokeWidth: 2 },
    label: e.type === 'parallel' ? 'song song' : undefined,
  }));
  return { rfNodes, rfEdges };
}

function reactToFlow(rfNodes: Node[], rfEdges: Edge[]): { nodes: FlowNode[]; edges: FlowEdge[] } {
...
export default function ProcessPage() {
  ...
  useEffect(() => { if (isAuthenticated) fetchProcesses(); }, [isAuthenticated, fetchProcesses]);
  ...
  const loadProcess = (p: ProcessTemplate) => {
    ...
    const { rfNodes, rfEdges } = flowToReact(p.flowchart.nodes || [], p.flowchart.edges || []);
    setNodes(rfNodes);
    setEdges(rfEdges);
  ...
  const createProcess = async () => {
    ...
    if (res.ok) {
      ...
      const newP = { id: d.id, name: newName, description: null, flowchart: { nodes: [], edges: [] }, version: 1, is_active: true, created_at: '', updated_at: '' };
      loadProcess(newP);
    }
  };
  ...
  const saveProcess = async () => {
    ...
    try {
      await fetch(`/api/api/supply-chain/processes/${activeId}`, {
        method: 'PUT',
        headers: { ...headers, 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: processName, description: processDesc, flowchart: { nodes: flowNodes, edges: flowEdges } }),
      });
      setSaved(true);
  ...
  const deleteProcess = async (id: string) => {
    ...
    await fetch(`/api/api/supply-chain/processes/${id}`, { method: 'DELETE', headers });
    if (activeId === id) { setActiveId(null); setNodes([]); setEdges([]); }
    fetchProcesses();
  };
  ...
  const exportDocx = async () => {
    ...
    // Save first
    await saveProcess();
    const res = await fetch(`/api/api/supply-chain/processes/${activeId}/export-docx`, { method: 'POST', headers });
    if (!res.ok) { alert('Xuất file thất bại'); return; }
  ...
  const addNode = (type: 'main' | 'sub') => {
    ...
    const newNode: Node = {
      ...
      data: { label: type === 'main' ? 'Bước mới' : 'Bước phụ', nodeType: type, order: nodes.length + 1 },
    };
  ...
  const onConnect = useCallback((conn: Connection) => {
    setEdges(eds => addEdge({
      ...conn, type: 'smoothstep', style: { stroke: '#087653', strokeWidth: 2 },
    }, eds));
  }, [setEdges]);
  ...
  return (
    <div data-page className="flex flex-col flex-1 lg:min-h-0 w-full overflow-hidden">
      ...
      <div className="rounded-2xl p-5 mb-4 animate-section" style={{ background: '#F0F7F4', border: '1px solid #E2E8F0' }}>
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold" style={{ color: '#1A2332' }}>Quy trình sản xuất</h1>
          ...
          <div className="flex gap-2">
            {activeId && (
              <>
                <button onClick={exportDocx} className="px-4 py-2 rounded-lg text-xs font-medium transition-all"
                ...
                <button onClick={saveProcess} disabled={saving}
                  ...
                  style={{ background: saving ? '#E2E8F0' : '#087653' }}>
      ...
      <div className="flex-1 flex gap-4 min-h-0 overflow-hidden">
          ...
          {listLoading ? (
            ...
            processes.map((p, idx) => (
              <div key={p.id} className={`rounded-lg p-3 cursor-pointer transition-all animate-list-item stagger-${Math.min(idx + 1, 12)}`}
                style={{
                  background: activeId === p.id ? '#087653' : '#FFFFFF',
                  color: activeId === p.id ? '#FFFFFF' : '#1A2332',
                  border: `1px solid ${activeId === p.id ? '#087653' : '#E2E8F0'}`,
                }}
                onClick={() => loadProcess(p)}>
                <p className="text-sm font-medium truncate">{p.name}</p>
                <div className="flex items-center justify-between mt-1">
                  <span className="text-xs" style={{ color: activeId === p.id ? 'rgba(255,255,255,0.7)' : '#9CA3AF' }}>
                  ...
                  <button onClick={e => { e.stopPropagation(); deleteProcess(p.id); }}
                    ...
                    style={{ color: activeId === p.id ? 'rgba(255,255,255,0.5)' : '#9CA3AF' }}>
                    ✕
                  </button>
                </div>
              </div>
        ...
        <div className="flex-1 rounded-xl overflow-hidden" style={{ border: '1px solid #E2E8F0', background: '#FAFCF9' }}>
          {!activeId ? (
            <div className="h-full grid place-items-center">
              <div className="text-center animate-scale-in">
                <svg className="w-16 h-16 mx-auto mb-3 animate-empty-icon" style={{ color: '#E2E8F0' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <p className="text-sm font-medium" style={{ color: '#374151' }}>Chọn quy trình hoặc tạo mới</p>
                <p className="text-xs mt-1" style={{ color: '#9CA3AF' }}>Kéo thả các bước và nối chúng lại</p>
              </div>
            </div>
          ) : (
            <ReactFlow
        ...
        {selectedNode && activeId && (
          <div className="w-72 flex-shrink-0 overflow-y-auto rounded-xl p-4 space-y-3"
            ...
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold" style={{ color: '#1A2332' }}>Chi tiết bước</h3>
            ...
            {[
              ...
              { key: 'conditions', label: 'Nhiệt độ / Điều kiện', type: 'text' },
              { key: 'notes', label: 'Ghi chú', type: 'textarea' },
            ].map(f => (
              <div key={f.key}>
                <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>{f.label}</label>
                {f.type === 'textarea' ? (
                  <textarea value={selectedNode.data?.[f.key] || ''} onChange={e => updateNodeData(f.key, e.target.value)}
                    rows={2} className="w-full px-3 py-2 rounded-lg text-xs outline-none resize-none" style={inputStyle} />
                ) : (
                  <input value={selectedNode.data?.[f.key] || ''} onChange={e => updateNodeData(f.key, e.target.value)}
            ...
            {/* Checklist */}
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Checklist Halal</label>
              {(selectedNode.data?.checklist || []).map((item: string, i: number) => (
                <div key={i} className="flex items-center gap-1 mb-1">
                  ...
                  <button onClick={() => {
                    const list = (selectedNode.data?.checklist || []).filter((_: any, j: number) => j !== i);
                    updateNodeData('checklist', list);
                  }} className="text-xs" style={{ color: '#DC2626' }}>✕</button>
                </div>
              ))}
              <button onClick={() => updateNodeData('checklist', [...(selectedNode.data?.checklist || []), ''])}
                className="text-xs mt-1" style={{ color: '#087653' }}>+ Thêm item</button>
            </div>
          </div>
        )}
      </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-modal-overlay"
          ...
          <div className="w-full max-w-sm rounded-2xl p-6 animate-modal-content"
            ...
            <h3 className="text-base font-bold mb-4" style={{ color: '#1A2332' }}>Tạo quy trình mới</h3>
            ...
            <button onClick={createProcess} disabled={!newName.trim()}
              ...
              style={{ background: !newName.trim() ? '#E2E8F0' : '#087653', color: !newName.trim() ? '#9CA3AF' : '#fff' }}>
              Tạo
            </button>
          </div>
        </div>
