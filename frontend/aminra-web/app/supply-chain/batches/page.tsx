...
export default function BatchesPage() {
  ...
  const fetchBatches = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/api/supply-chain/batches?${params}`, {
        headers,
      });
      if (res.ok) {
  ...
  const fetchOptions = async () => {
    ...
    if (pRes.ok) {
      ...
      setProcesses(
        d.processes?.map((p: any) => ({ id: p.id, name: p.name })) || [],
      );
    }
    if (mRes.ok) {
  ...
  const downloadQR = async (bid: string) => {
    const res = await fetch(`/api/api/supply-chain/batches/${bid}/qr`, {
      headers,
    });
    if (!res.ok) {
        ...
        {loading ? (
          ...
          >
            ...
            <p className="text-sm mt-1" style={{ color: "#6B7280" }}>
              Tạo quy trình trước, sau đó tạo lô hàng
            </p>
          </div>
        ) : (
          batches.map((b, idx) => {
            ...
            const progress =
              b.step_count > 0
                ? Math.round((b.step_completed / b.step_count) * 100)
                : 0;
            return (
              <div
                key={b.id}
                className={`rounded-xl p-4 doc-card-hover animate-list-item stagger-${Math.min(idx + 1, 12)} cursor-pointer`}
                style={cardStyle}
                    ...
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      {b.compliance_score != null && (
                        <span
                          ...
                          style={{
                            color:
                              b.compliance_score >= 75
                                ? "#102A5C"
                                : b.compliance_score >= 50
                                  ? "#B45309"
                                  : "#DC2626",
                          }}
                        >
          ...
          >
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="VD: LOT-20260409-001"
              ...
              <div>
                ...
                <select
                  value={createForm.process_template_id}
                  onChange={(e) =>
                    setCreateForm((f) => ({
                      ...f,
                      process_template_id: e.target.value,
                    }))
                  }
                  className="w-full px-4 py-2.5 rounded-lg text-sm outline-none"
                  style={inputStyle}
              ...
              <button
                ...
                disabled={creating || !createForm.product_name}
                className="w-full py-2.5 rounded-xl text-sm font-semibold text-white"
                style={{
                  background: !createForm.product_name ? "#E2E8F0" : "#0A1F44",
                  color: !createForm.product_name ? "#9CA3AF" : "#fff",
                }}
              >
          ...
          >
            {detailLoading ? (
              <div className="py-16 text-center">
                <div className="w-8 h-8 border-2 border-[#0A1F44] border-t-transparent rounded-full animate-spin mx-auto" />
              </div>
            ) : (
              detail && (
                <>
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h3
                          ...
                          style={{ color: "#0A1F44" }}
                        >
                          {detail.batch.batch_code}
                        </h3>
                        {(() => {
                          const st =
                            STATUS[detail.batch.status as keyof typeof STATUS];
                          return st ? (
                            <span
                              className="text-xs px-2 py-0.5 rounded animate-chip"
                              style={{ background: st.bg, color: st.color }}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {detail.batch.integrity_hash && (
                        <button
                          onClick={async () => {
                            const res = await fetch(
                              `/api/api/supply-chain/batches/${detail.batch.id}/verify`,
                              { headers },
                            );
                            if (res.ok) {
                              const d = await res.json();
                  ...
                  >
                    {/* Batch timeline */}
                    <div
                      className="mb-4 flex flex-wrap gap-x-4 gap-y-1 text-xs"
                      style={{ color: "#6B7280" }}
                    >
                      {detail.batch.started_at && (
                        <span>
                          Bắt đầu:{" "}
                          <strong style={{ color: "#374151" }}>
                            {new Date(detail.batch.started_at).toLocaleString(
                              "vi-VN",
                            )}
                          </strong>
                        </span>
                      )}
                      {detail.batch.completed_at && (
                        <span>
                          Hoàn thành:{" "}
                          <strong style={{ color: "#374151" }}>
                            {new Date(detail.batch.completed_at).toLocaleString(
                              "vi-VN",
                            )}
                          </strong>
                        </span>
                      ...
                      {detail.batch.approved_by && (
                        <span style={{ color: "#102A5C" }}>
                          Xác nhận bởi:{" "}
                          <strong>{detail.batch.approved_by}</strong> ·{" "}
                          {new Date(detail.batch.approved_at).toLocaleString(
                            "vi-VN",
                          )}
                        </span>
                      )}
                      {detail.batch.compliance_score != null && (
                        <span>
                          ...
                          <strong
                            style={{
                              color:
                                detail.batch.compliance_score >= 100
                                  ? "#102A5C"
                                  : "#B45309",
                            }}
                          >
                            {detail.batch.compliance_score}%
                          </strong>
                        </span>
                      )}
                      {detail.batch.integrity_hash && (
                        <span
                          className="px-2 py-0.5 rounded text-xs font-medium animate-chip"
                          style={{
                            background: "#DCE3F0",
                            color: "#102A5C",
                            border: "1px solid #D9B96E",
                        ...
                        >
                          SEALED ·{" "}
                          {detail.batch.integrity_hash.substring(0, 12)}...
                        </span>
                      )}
                    </div>
                        ...
                        <div className="flex items-center gap-3">
                          <div className="flex-1">
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                if (e.target.value)
                                  assignMember(detail.batch.id, e.target.value);
                              }}
                              disabled={assigning}
                    ...
                    {detail.batch.assigned_name &&
                      detail.batch.integrity_hash && (
                        ...
                        >
                          Ủy quyền:{" "}
                          <strong>{detail.batch.assigned_name}</strong>
                        </div>
                      )}
                      ...
                      <div className="mb-4">
                        ...
                        <div className="space-y-2">
                          {detail.steps.map((step: any, i: number) => (
                            <div
                              key={step.id}
                              className={`rounded-lg p-3 animate-list-item stagger-${Math.min(i + 1, 12)}`}
                              style={{
                                background: step.approved_by
                                  ? "#F0FDF4"
                                  ...
                                  <input
                                    ...
                                    onBlur={(e) => {
                                      if (
                                        ...
                                        updateStep(detail.batch.id, step.id, {
                              ...
                              >
                                ...
                                {step.photo_path && (
                                  <button
                                    onClick={() =>
                                      openAuthed(
                                        `/api/api/supply-chain/batches/${detail.batch.id}/steps/${step.id}/photo/view`,
                                        token || "",
                                      )
                                    }
                    ...
                    {detail.materials?.length > 0 && (
                      <div>
                        <h4
                          ...
                          style={{ color: "#0A1F44" }}
                        >
                          Nguyên liệu ({detail.materials.length})
                        </h4>
                          ...
                          {detail.materials.map((m: any) => (
                            ...
                            >
                              <span style={{ color: "#374151" }}>
                                {m.material_name} {m.sku && `(${m.sku})`}
                              </span>
                              <span style={{ color: "#9CA3AF" }}>
                                {m.quantity} {m.unit} · {m.supplier_name}
                              </span>
                            </div>
                          ))}
                        </div>
