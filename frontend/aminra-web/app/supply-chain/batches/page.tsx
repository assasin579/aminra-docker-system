...
const STATUS = {
  ...
  completed:   { label: 'Hoàn thành', bg: '#DCE3F0', color: '#102A5C' },
      ...
      <div className="rounded-2xl p-5 mb-4 animate-section" style={{ background: '#F5F1E8', border: '1px solid #E2E8F0' }}>
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold" style={{ color: '#0A1F44' }}>Lô hàng</h1>
              <p className="text-xs" style={{ color: '#6B7280' }}>{batches.length} lô hàng</p>
            </div>
          </div>
          <button onClick={openCreate} className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white" style={{ background: '#0A1F44' }}>
            + Tạo lô hàng
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 lg:min-h-0 lg:overflow-y-auto space-y-3">
        {loading ? (
          ...
          <div className="rounded-2xl p-12 text-center animate-scale-in" style={cardStyle}>
            ...
            <p className="font-semibold" style={{ color: '#0A1F44' }}>Chưa có lô hàng nào</p>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <span className="text-sm font-bold" style={{ color: '#0A1F44' }}>{b.batch_code}</span>
                      ...
                      {b.compliance_score != null && (
                        <span className="text-xs font-bold" style={{ color: b.compliance_score >= 75 ? '#102A5C' : b.compliance_score >= 50 ? '#B45309' : '#DC2626' }}>
                          {b.compliance_score}%
                        </span>
                      )}
                    </div>
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        <div className="flex-1 h-1.5 rounded-full" style={{ background: '#E2E8F0' }}>
                          <div className="h-full rounded-full transition-all animate-progress"
                            style={{ width: `${progress}%`, background: progress >= 100 ? '#102A5C' : '#0A1F44' }} />
                        </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-modal-overlay"
          ...
          <div className="w-full max-w-lg rounded-2xl p-6 animate-modal-content overflow-y-auto"
            ...
            <h3 className="text-base font-bold mb-4" style={{ color: '#0A1F44' }}>Tạo lô hàng mới</h3>
            <div className="space-y-4">
              ...
              <button onClick={handleCreate} disabled={creating || !createForm.product_name}
                ...
                style={{ background: !createForm.product_name ? '#E2E8F0' : '#0A1F44', color: !createForm.product_name ? '#9CA3AF' : '#fff' }}>
      ...
      {(detail || detailLoading) && (
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-modal-overlay"
          ...
          <div className="w-full max-w-2xl rounded-2xl animate-modal-content overflow-hidden"
            style={{ ...cardStyle, maxHeight: 'calc(100vh - 4rem)' }} onClick={e => e.stopPropagation()}>
            {detailLoading ? (
              <div className="py-16 text-center"><div className="w-8 h-8 border-2 border-[#0A1F44] border-t-transparent rounded-full animate-spin mx-auto" /></div>
            ) : detail && (
              <>
                {/* Header */}
                <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid #E2E8F0', background: '#F5F1E8' }}>
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-base font-bold" style={{ color: '#0A1F44' }}>{detail.batch.batch_code}</h3>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {detail.batch.status === 'in_progress' && (
                      <button onClick={() => updateBatchStatus(detail.batch.id, 'completed')}
                        className="px-3 py-1.5 rounded-lg text-xs font-medium text-white" style={{ background: '#102A5C' }}>
                        Hoàn thành
                      </button>
                    )}
                    {detail.batch.status === 'completed' && !detail.batch.integrity_hash && (
                      <button onClick={async () => {
                        ...
                        className="px-3 py-1.5 rounded-lg text-xs font-medium text-white" style={{ background: '#0A1F44' }}>
                        Seal & Xác nhận
                      </button>
                    )}
                    {detail.batch.integrity_hash && (
                      <button onClick={async () => {
                        ...
                        style={{ background: '#DCE3F0', color: '#102A5C', border: '1px solid #D9B96E' }}>
                        Xác minh toàn vẹn
                      </button>
                    )}
                ...
                <div className="px-6 py-4 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 16rem)' }}>
                  ...
                  <div className="mb-4 flex flex-wrap gap-x-4 gap-y-1 text-xs" style={{ color: '#6B7280' }}>
                    ...
                    {detail.batch.compliance_score != null && (
                      <span>Tiến độ: <strong style={{ color: detail.batch.compliance_score >= 100 ? '#102A5C' : '#B45309' }}>{detail.batch.compliance_score}%</strong></span>
                    )}
                    {detail.batch.integrity_hash && (
                      <span className="px-2 py-0.5 rounded text-xs font-medium animate-chip" style={{ background: '#DCE3F0', color: '#102A5C', border: '1px solid #D9B96E' }}>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex-1">
                          ...
                          <select
                            ...
                            style={{ background: '#FFFFFF', border: '1px solid #DDD6FE', color: '#0A1F44' }}>
                  ...
                  {detail.steps?.length > 0 && (
                    <div className="mb-4">
                      <h4 className="text-sm font-bold mb-3" style={{ color: '#0A1F44' }}>
                      ...
                      <div className="space-y-2">
                        {detail.steps.map((step: any, i: number) => (
                          <div key={step.id} className={`rounded-lg p-3 animate-list-item stagger-${Math.min(i + 1, 12)}`}
                            style={{
                              background: step.approved_by ? '#F0FDF4' : step.status === 'completed' ? '#DCE3F0' : '#FFFFFF',
                              border: `1px solid ${step.approved_by ? '#D9B96E' : '#E2E8F0'}`,
                            }}>
                            {/* Row 1: Step name + status */}
                            <div className="flex items-center gap-2 mb-2">
                              <span className="text-xs font-bold px-2 py-0.5 rounded"
                                style={{ background: step.status === 'completed' ? '#102A5C' : '#E2E8F0', color: step.status === 'completed' ? '#fff' : '#6B7280' }}>
                                #{i + 1}
                              </span>
                              <span className="text-sm font-medium" style={{ color: '#0A1F44' }}>{step.step_name}</span>
                              {step.approved_by && (
                                <svg className="w-4 h-4 ml-auto" style={{ color: '#102A5C' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            {/* Row 4: Approval */}
                            {step.approved_by ? (
                              <div className="flex items-center gap-1 mt-2 text-xs" style={{ color: '#102A5C' }}>
                  ...
                  {detail.materials?.length > 0 && (
                    <div>
                      <h4 className="text-sm font-bold mb-2" style={{ color: '#0A1F44' }}>Nguyên liệu ({detail.materials.length})</h4>
