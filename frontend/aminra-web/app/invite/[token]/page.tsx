...
export default function InviteAcceptPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (success) return (
    <div className="min-h-screen grid place-items-center p-6" style={{ background: '#F0F7F4' }}>
      <div className="max-w-md text-center">
        <div className="w-16 h-16 rounded-full grid place-items-center mx-auto mb-4" style={{ background: '#ECFDF5' }}>
          <svg className="w-8 h-8" style={{ color: '#059669' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
          </svg>
        </div>
        <h1 className="text-xl font-bold mb-2" style={{ color: '#1A2332' }}>Tham gia thành công!</h1>
        <p className="text-sm mb-6" style={{ color: '#6B7280' }}>
          Bạn đã gia nhập <strong>{info?.business_name}</strong>. Đăng nhập để bắt đầu.
        </p>
        <Link href="/business/login"
          className="inline-block px-6 py-3 rounded-xl font-semibold text-white" style={{ background: '#087653' }}>
          Đăng nhập ngay
        </Link>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen grid place-items-center p-6" style={{ background: '#F0F7F4' }} data-page>
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8 animate-section">
          ...
          <h1 className="text-2xl font-bold" style={{ color: '#1A2332' }}>Lời mời tham gia</h1>
        ...
        <div className="rounded-2xl p-5 mb-6 animate-section" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium" style={{ color: '#1A2332' }}>{info.email}</span>
            </div>
            {info.role && (
              <div className="flex justify-between">
                <span style={{ color: '#6B7280' }}>Vai trò</span>
                <span className="font-medium" style={{ color: '#1A2332' }}>{info.role}</span>
              </div>
            )}
            {info.department && (
              <div className="flex justify-between">
                <span style={{ color: '#6B7280' }}>Phòng ban</span>
                <span className="font-medium" style={{ color: '#1A2332' }}>{info.department}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="rounded-2xl p-6 animate-section" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={submitting || !password || !confirmPw}
              ...
              style={{ background: (!password || !confirmPw) ? '#E2E8F0' : '#087653', color: (!password || !confirmPw) ? '#9CA3AF' : '#fff' }}>
