...
export default function InviteAcceptPage({
  ...
  useEffect(() => {
    fetch(`/api/auth/invite/${token}`)
      .then((r) => {
        if (r.status === 410)
          throw new Error("Link đã hết hạn hoặc đã được sử dụng.");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 10) {
      setSubmitError("Mật khẩu phải có ít nhất 10 ký tự");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/auth/invite/${token}/accept`, {
        ...
        body: JSON.stringify({
          password,
          display_name: displayName || info?.email,
        }),
      });
      if (!res.ok) {
      ...
      >
        <div className="max-w-md text-center">
          ...
          <p className="text-sm mb-6" style={{ color: "#6B7280" }}>
            Bạn đã gia nhập <strong>{info?.business_name}</strong>. Đăng nhập để
            bắt đầu.
          </p>
          <Link
            href="/business/login"
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8 animate-section">
          ...
          <p className="text-sm mt-2" style={{ color: "#6B7280" }}>
            ...
            mời bạn tham gia tổ chức trên AMINRA
          </p>
        </div>

        {/* Info */}
        <div
          className="rounded-2xl p-5 mb-6 animate-section"
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              ...
              style={{
                background: !password || !confirmPw ? "#E2E8F0" : "#0A1F44",
                color: !password || !confirmPw ? "#9CA3AF" : "#fff",
              }}
            >
