...
export default function CreateDocumentPage() {
  ...
  const generateDocx = useCallback(async (docType: string) => {
    ...
    const res = await fetch(`/api/templates/${docType}/download?lang=${lang}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    return (
      <div className="min-h-screen grid place-items-center" style={{ background: '#FFFFFF' }}>
        <div className="text-center space-y-4">
          <p style={{ color: '#5F6F80' }}>Vui lòng đăng nhập để sử dụng chức năng này</p>
  ...
  if (user?.role !== 'business') {
    return (
      <div className="min-h-screen grid place-items-center" style={{ background: '#FFFFFF' }}>
        <p style={{ color: '#5F6F80' }}>Chức năng này chỉ dành cho tài khoản doanh nghiệp</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen" style={{ background: '#FFFFFF' }} data-page>
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="animate-section">
          <h1 className="text-xl font-bold" style={{ color: '#1A2332' }}>Tạo hồ sơ theo đúng chuẩn</h1>
          <p className="text-sm mt-1" style={{ color: '#5F6F80' }}>
            ...
            <div key={s.n} className="contents">
              <div className="grid items-center" style={{ gridTemplateColumns: 'auto auto', gap: '8px' }}>
                <div className="w-8 h-8 rounded-full grid place-items-center text-xs font-bold"
                  style={{
                    background: step >= s.n ? '#087653' : '#E2E8F0',
                    color: step >= s.n ? '#fff' : '#5B6B7D',
                  }}>
                  {s.n}
                </div>
                <span className="text-sm font-medium" style={{ color: step >= s.n ? '#1A2332' : '#5B6B7D' }}>
                  {s.label}
                </span>
              </div>
              {i < 1 && (
                <div className="h-0.5 w-12" style={{ background: step > s.n ? '#087653' : '#E2E8F0' }} />
              )}
            </div>
          ))}
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl p-5" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
              <div className="grid items-center mb-3" style={{ gridTemplateColumns: '1fr auto' }}>
                <h3 className="text-sm font-semibold" style={{ color: '#1A2332' }}>Thông tin doanh nghiệp</h3>
              ...
              <div className="grid gap-2 grid-cols-1 sm:grid-cols-2">
                {profileRows.map(r => (
                  <div key={r.label} className="text-xs">
                    <span style={{ color: '#5B6B7D' }}>{r.label}: </span>
                    {r.value ? (
                      <span className="font-medium" style={{ color: '#1A2332' }}>{r.value}</span>
                    ) : (
                ...
                <div className="text-xs">
                  <span style={{ color: '#5B6B7D' }}>Ngày tạo: </span>
                  <span className="font-medium" style={{ color: '#1A2332' }}>{dateStr}</span>
                </div>
              </div>
            </div>
            ...
            {/* Template list */}
            <div>
              <h2 className="text-base font-bold mb-1" style={{ color: '#1A2332' }}>Chọn mẫu tài liệu</h2>
              <p className="text-xs mb-4" style={{ color: '#5F6F80' }}>
              ...
              {templates.length === 0 ? (
                <div className="rounded-2xl p-8 text-center" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
                  <p className="text-sm font-semibold mb-1" style={{ color: '#1A2332' }}>Chưa có mẫu tài liệu</p>
                  <p className="text-xs" style={{ color: '#5F6F80' }}>Admin cần upload template DOCX tại trang quản lý.</p>
                </div>
              ) : (
                  ...
                  {templates.map(t => {
                    ...
                    return (
                      <button key={t.doc_type} onClick={() => toggleTemplate(t.doc_type)} disabled={disabled}
                        ...
                        style={{
                          background: selected ? 'rgba(8,118,83,0.06)' : '#FFFFFF',
                          border: `2px solid ${selected ? '#087653' : '#E2E8F0'}`,
                        ...
                        <div className="grid" style={{ gridTemplateColumns: 'auto 1fr', gap: '12px' }}>
                          <div className="w-5 h-5 rounded grid place-items-center mt-0.5"
                            style={{
                              border: `2px solid ${selected ? '#087653' : '#E2E8F0'}`,
                              background: selected ? '#087653' : 'transparent',
                            }}>
                            {selected && (
                          ...
                          <div>
                            <div className="text-sm font-semibold" style={{ color: '#1A2332' }}>{t.label}</div>
                            <div className="text-xs mt-1" style={{ color: '#5B6B7D' }}>
            ...
            <div className="grid justify-end">
              <button
                ...
                style={{
                  background: selectedTypes.length > 0 && profileComplete ? '#087653' : '#E2E8F0',
                  color: selectedTypes.length > 0 && profileComplete ? '#fff' : '#5B6B7D',
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            {/* Language selection */}
            <div className="rounded-2xl p-5" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
              <h3 className="text-xs font-medium mb-3" style={{ color: '#5F6F80' }}>Ngôn ngữ tài liệu</h3>
                ...
                {([
                  ...
                  return (
                    <button key={l.id} onClick={() => available && setLang(l.id)}
                      ...
                      style={{
                        background: lang === l.id ? 'rgba(8,118,83,0.06)' : '#FAFCF9',
                        border: `2px solid ${lang === l.id ? '#087653' : '#E2E8F0'}`,
                        opacity: available ? 1 : 0.35,
                        cursor: available ? 'pointer' : 'not-allowed',
                      }}>
                      <div className="text-lg mb-1">{l.flag}</div>
                      <div className="text-sm font-semibold" style={{ color: '#1A2332' }}>{l.label}</div>
                      {!available && (
            ...
            <div className="rounded-2xl p-5" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
              <div className="mb-4">
                <h3 className="text-xs font-medium mb-2" style={{ color: '#5F6F80' }}>Tài liệu sẽ tạo</h3>
                <div className="flex flex-wrap gap-2">
                  {selectedTypes.map(dt => {
                    const t = templates.find(tpl => tpl.doc_type === dt);
                    return (
                      <span key={dt} className="px-3 py-1 rounded-full text-xs font-medium"
                        style={{ background: 'rgba(8,118,83,0.1)', color: '#087653' }}>
                        {t?.label || dt}
                      </span>
                    );
                  })}
              ...
              <div style={{ borderTop: '1px solid #E2E8F0', paddingTop: '16px' }}>
                <h3 className="text-xs font-medium mb-2" style={{ color: '#5F6F80' }}>Thông tin sẽ điền vào template</h3>
                <div className="grid gap-2 grid-cols-1 sm:grid-cols-2">
            ...
            {generated && (
              <div className="px-4 py-3 rounded-xl text-sm"
                style={{ background: 'rgba(8,118,83,0.08)', border: '1px solid rgba(8,118,83,0.2)', color: '#087653' }}>
            ...
            <div className="grid items-center" style={{ gridTemplateColumns: 'auto 1fr', gap: '12px' }}>
              <button onClick={() => { setStep(1); setGenerated(false); }}
                className="px-5 py-3 rounded-xl text-sm" style={{ color: '#5B6B7D' }}>
                Quay lại
              </button>
              ...
              <button onClick={handleGenerate} disabled={generating}
                ...
                style={{
                  background: generating ? '#E2E8F0' : '#087653',
                  color: '#fff',
                  cursor: generating ? 'not-allowed' : 'pointer',
                  boxShadow: generating ? 'none' : '0 4px 20px rgba(8,118,83,0.35)',
                }}>
                {generating ? (
            ...
            {/* Info */}
            <div className="rounded-xl p-4 text-xs" style={{ background: '#F0F7F4', border: '1px solid #E2E8F0', color: '#5B6B7D' }}>
              <strong style={{ color: '#3D4F5F' }}>Cách hoạt động:</strong>
