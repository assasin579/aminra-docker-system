...
export default function SettingsPage() {
  ...
  const [notif, setNotif] = useState({
    notify_eval_done: true,
    notify_submission_reply: true,
  });
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetch("/api/auth/notification-preferences", { headers }).then((r) =>
        r.ok ? r.json() : null,
      ),
    ])
      .then(([notifData]) => {
        ...
        if (user?.tenant_id)
          setLogoUrl(`/api/auth/company-logo/${user.tenant_id}`);
      })
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(e.detail || "Đổi mật khẩu thất bại");
      }
      ...
      setPwMsg({
        type: "err",
        text: err instanceof Error ? err.message : "Lỗi",
      });
    } finally {
      setPwSaving(false);
    }
  ...
  return (
    <div data-page style={{ background: "#FFFFFF" }}>
      <div className="max-w-xl mx-auto px-4 py-8 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: "#6B7280" }}>
                Bảo mật, thông báo và cá nhân hóa
              </p>
            </div>
          </div>
        </div>
        ...
        {isOwner && (
          <div
            className="animate-section rounded-2xl p-6 doc-card-hover"
            style={cardStyle}
          >
            <h2
            ...
            <div className="flex items-center gap-4">
              <div
                className="w-20 h-20 rounded-xl grid place-items-center overflow-hidden"
        ...
        {/* ── Đổi mật khẩu ── */}
        <div
          className="animate-section rounded-2xl p-6 space-y-4 doc-card-hover"
          style={cardStyle}
        >
          ...
          <div>
            ...
            <input
              ...
              placeholder="Tối thiểu 10 ký tự, gồm hoa, thường, số"
              className="w-full px-4 py-2.5 rounded-lg text-sm outline-none"
              style={inputStyle}
            />
          </div>
          ...
          <button
            ...
            disabled={
              pwSaving || !pwForm.current || !pwForm.newPw || !pwForm.confirm
            }
        ...
        >
          ...
          {[
            {
              key: "notify_eval_done" as const,
              label: "Đánh giá tài liệu hoàn tất",
              desc: "Nhận thông báo khi AI hoàn thành đánh giá",
            },
            {
              key: "notify_submission_reply" as const,
              label: "Phản hồi hồ sơ",
              desc: "Nhận thông báo khi tổ chức chứng nhận phản hồi",
            },
          ].map((item) => (
            <div
              key={item.key}
          ...
          <button
            ...
            className="w-full py-2.5 rounded-xl font-semibold text-sm transition-all text-white"
            style={{ background: notifSaving ? "#E2E8F0" : "#0A1F44" }}
          >
        ...
        >
          ...
          <p className="text-xs" style={{ color: "#6B7280" }}>
            Theo Nghị định 13/2023/NĐ-CP và GDPR Article 20, bạn có quyền yêu
            cầu một bản sao toàn bộ dữ liệu cá nhân chúng tôi đang lưu trữ về
            bạn.
          </p>
          <Link
