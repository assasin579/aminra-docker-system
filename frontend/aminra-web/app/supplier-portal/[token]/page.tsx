...
interface PortalInfo {
  ...
  uploaded_certificates: { id: string; cert_type: string; filename: string; size: number; uploaded_at: string }[];
}
...
export default function SupplierPortalPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const fetchInfo = async () => {
    ...
    try {
      ...
      if (res.status === 410) { setError('Link đã hết hạn. Vui lòng liên hệ doanh nghiệp để nhận link mới.'); return; }
      if (!res.ok) { setError('Link không hợp lệ.'); return; }
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      const res = await fetch(`/api/api/supply-chain/supplier-portal/${token}/upload`, { method: 'POST', body: fd });
    ...
    <div className="min-h-screen" style={{ background: '#F5F1E8' }} data-page>
      <div className="max-w-xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="text-center animate-section">
          ...
          <h1 className="text-2xl font-bold" style={{ color: '#0A1F44' }}>Gửi hồ sơ nhà cung cấp</h1>
        ...
        <div className="rounded-2xl p-6 animate-section" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
          <h2 className="text-sm font-bold mb-4" style={{ color: '#0A1F44' }}>Hồ sơ yêu cầu</h2>
            ...
            {info.required_documents.map(doc => {
              ...
              return (
                <div key={doc.type} className="flex items-center gap-3 px-3 py-2.5 rounded-lg"
                  style={{ background: done ? '#DCE3F0' : '#FFFFFF', border: `1px solid ${done ? '#D9B96E' : '#E2E8F0'}` }}>
                  <div className="w-5 h-5 rounded grid place-items-center flex-shrink-0"
                    style={{ background: done ? '#102A5C' : '#E2E8F0', border: `2px solid ${done ? '#102A5C' : '#D1D5DB'}` }}>
                    {done && <svg width="10" height="10" fill="none" viewBox="0 0 24 24" stroke="#fff" strokeWidth={4}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                    </svg>}
                  </div>
                  <span className="text-sm flex-1" style={{ color: done ? '#102A5C' : '#374151' }}>{doc.label}</span>
                  {doc.required && !done && (
                    <span className="text-xs px-2 py-0.5 rounded" style={{ background: '#FEF2F2', color: '#DC2626' }}>Bắt buộc</span>
                  )}
                  {done && <span className="text-xs" style={{ color: '#102A5C' }}>Đã gửi</span>}
                </div>
              );
            })}
          </div>
        ...
        {/* Upload section */}
        <div className="rounded-2xl p-6 animate-section" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
          <h2 className="text-sm font-bold mb-4" style={{ color: '#0A1F44' }}>Gửi hồ sơ</h2>
          <div className="space-y-3">
            ...
            <button onClick={() => fileRef.current?.click()} disabled={uploading}
              ...
              style={{ background: uploading ? '#E2E8F0' : '#0A1F44', color: uploading ? '#9CA3AF' : '#fff' }}>
        ...
        {info.uploaded_certificates.length > 0 && (
          <div className="rounded-2xl p-6 animate-section" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
            <h2 className="text-sm font-bold mb-3" style={{ color: '#0A1F44' }}>Hồ sơ đã gửi ({info.uploaded_certificates.length})</h2>
            <div className="space-y-2">
              {info.uploaded_certificates.map(c => (
                <div key={c.id} className="flex items-center gap-3 px-3 py-2.5 rounded-lg"
                  style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
                  <div className="w-8 h-8 rounded-lg grid place-items-center" style={{ background: '#DCE3F0' }}>
                    <svg className="w-4 h-4" style={{ color: '#102A5C' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
                    </svg>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate" style={{ color: '#374151' }}>{c.filename}</p>
                    <p className="text-xs" style={{ color: '#9CA3AF' }}>
                      {(c.size / 1024).toFixed(0)} KB · {new Date(c.uploaded_at).toLocaleDateString('vi-VN')}
                    </p>
                  </div>
                </div>
              ))}
