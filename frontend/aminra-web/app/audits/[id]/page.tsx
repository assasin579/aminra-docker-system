...
export default function AuditVisitDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const btnPrimary = "w-full py-3.5 rounded-xl border-none bg-[#0A1F44] text-white text-base font-bold cursor-pointer transition-all active:scale-[0.98]";
  ...
  const inputCls = "w-full px-3 py-2.5 rounded-lg border border-[#E2E8F0] text-sm outline-none transition-all focus:border-[rgba(10,31,68,0.4)] focus:shadow-[0_0_0_3px_rgba(10,31,68,0.08)]";
  ...
  const renderOverview = () => (
    <div className="animate-tab-content">
      {/* Visit Info */}
      <div className={card}>
        <h2 className="text-lg font-bold mb-2" style={{ color: '#0A1F44' }}>{visit.business_name}</h2>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <button className={`${btnOutline} ${visit.start_gps ? 'text-[#0A1F44] border-[#0A1F44]' : ''}`} onClick={() => captureGPS('start')}>
          {visit.start_gps ? '✓ GPS bắt đầu' : 'Ghi GPS bắt đầu'}
        </button>
        <button className={`${btnOutline} ${visit.end_gps ? 'text-[#0A1F44] border-[#0A1F44]' : ''}`} onClick={() => captureGPS('end')}>
      ...
      {visit.compliance_score != null && (
        <div className={`${card} text-center`}>
          <div className="w-24 h-24 rounded-full mx-auto mb-2 flex items-center justify-center text-2xl font-bold"
            style={{ border: `6px solid ${visit.compliance_score >= 80 ? '#0A1F44' : visit.compliance_score >= 60 ? '#D97706' : '#DC2626'}`,
                     color: visit.compliance_score >= 80 ? '#0A1F44' : visit.compliance_score >= 60 ? '#D97706' : '#DC2626' }}>
      ...
      {(visit.status === 'in_progress' || visit.status === 'completed') && (
        <div className={card}>
          <h3 className="text-base font-bold mb-3" style={{ color: '#0A1F44' }}>Chữ ký</h3>
          {(['auditor', 'business'] as const).map(type => {
            ...
            return (
              <div key={type} className="mb-4">
                <label className={labelCls}>{type === 'auditor' ? 'Kiểm tra viên' : 'Doanh nghiệp'} {done && <span className="text-[#0A1F44]">✓ Đã ký</span>}</label>
                {!done && (
                  <>
  ...
  const renderChecklist = () => (
    <div className="animate-tab-content">
      ...
      {/* Actions */}
      <div className="flex flex-wrap gap-2 mb-3">
        <button className={`${btnOutline} ${showAddItem ? '!text-red-500 !border-red-300' : '!text-[#0A1F44] !border-[#0A1F44]'}`}
      ...
      {Object.entries(grouped).map(([cat, catItems]) => (
        <div key={cat} className="mb-4">
          <h3 className="text-sm font-bold py-2 mb-2 border-b-2 border-[#E2E8F0]" style={{ color: '#0A1F44' }}>{cat}</h3>
          {catItems.map(item => {
            ...
            <div key={item.id} className={`${card} doc-card-hover`}>
              ...
              <div className="flex items-start gap-2 mb-2">
                ...
                <span className="flex-1 text-sm leading-snug" style={{ color: '#0A1F44' }}>{item.criteria}</span>
    ...
    <div className="animate-tab-content">
      ...
      {ncrs.map(ncr => (
        <div key={ncr.id} className={`${card} doc-card-hover`}>
          ...
          <p className="text-sm mb-2 leading-relaxed" style={{ color: '#0A1F44' }}>{ncr.description}</p>
  ...
  const renderReport = () => (
    <div className="animate-tab-content">
      <div className={card}>
        <h3 className="text-base font-bold mb-3" style={{ color: '#0A1F44' }}>Báo cáo PDF</h3>
      ...
      {user?.is_owner && (
        <div className={card}>
          <h3 className="text-base font-bold mb-3" style={{ color: '#0A1F44' }}>Quyết định chứng nhận</h3>
  ...
  return (
    <div className="flex flex-col flex-1 lg:min-h-0 w-full" data-page>
      ...
      <div className="flex items-center gap-3 py-4">
        ...
        <h1 className="text-lg font-bold" style={{ color: '#0A1F44' }}>Chi tiết kiểm tra</h1>
      </div>

      {/* Tabs */}
      <div className="grid grid-cols-4 gap-1 mb-4 p-1 rounded-xl" style={{ background: '#F5F1E8', border: '1px solid #E2E8F0' }}>
        {TABS.map(t => (
