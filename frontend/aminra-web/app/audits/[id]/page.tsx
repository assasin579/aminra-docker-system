...
interface Visit {
  ...
  report_pdf_path: boolean;
  start_gps: { lat: number; lng: number } | null;
  end_gps: { lat: number; lng: number } | null;
  has_auditor_sig: boolean;
  has_business_sig: boolean;
  created_at: string;
}
...
const VISIT_TYPE: Record<string, { label: string; bg: string; color: string }> =
  {
    ...
    renewal: { label: "Gia hạn", bg: "#DCE3F0", color: "#0A1F44" },
...
const STATUS: Record<string, { label: string; bg: string; color: string }> = {
  ...
  completed: { label: "Hoàn thành", bg: "#DCE3F0", color: "#0A1F44" },
...
const NCR_STATUS: Record<string, { label: string; bg: string; color: string }> =
  {
    ...
    closed: { label: "Đã đóng", bg: "#DCE3F0", color: "#0A1F44" },
  };
const RESULTS = [
  { value: "conform", label: "C (Đạt)", color: "#0A1F44" },
...
/* ── Main Component ── */
export default function AuditVisitDetailPage({
  params,
}: {
  ...
  if (!isAuthenticated || !user || user.role !== "provider") {
    return (
      <div
        className="grid place-items-center min-h-[60vh] text-sm"
        style={{ color: "#6B7280" }}
      >
  ...
  const addItem = async () => {
    if (!newItem.code || !newItem.criteria)
      return alert("Cần nhập mã và tiêu chí");
  ...
  const uploadItemPhoto = async (itemId: string, file: File) => {
    ...
    fd.append("file", file);
    await fetch(`/api/api/audits/${id}/items/${itemId}/photo`, {
      method: "POST",
  ...
  const uploadNcrPhoto = async (ncrId: string, file: File) => {
    const fd = new FormData();
    fd.append("file", file);
    await fetch(`/api/api/audits/${id}/ncr/${ncrId}/photo`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
      body: fd,
    });
  ...
  const uploadSignature = async (
    ...
    canvas.toBlob(async (blob) => {
      if (!blob) return;
      const fd = new FormData();
      fd.append("file", blob, `${type}-signature.png`);
      await fetch(`/api/api/audits/${id}/signature?type=${type}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: fd,
      });
  ...
  items.forEach((it) => {
    if (it.result && it.result in counts)
      counts[it.result as keyof typeof counts]++;
  });

  /* ── Style helpers (Tailwind class strings) ── */
  const card = "bg-white rounded-xl p-4 mb-3 border border-[#E2E8F0]";
  const btnPrimary =
    "w-full py-3.5 rounded-xl border-none bg-[#0A1F44] text-white text-base font-bold cursor-pointer transition-all active:scale-[0.98]";
  const btnOutline =
    "px-4 py-2.5 rounded-lg border-[1.5px] border-[#E2E8F0] bg-white text-sm font-semibold cursor-pointer transition-all active:scale-[0.98] text-center";
  const inputCls =
    "w-full px-3 py-2.5 rounded-lg border border-[#E2E8F0] text-sm outline-none transition-all focus:border-[rgba(10,31,68,0.4)] focus:shadow-[0_0_0_3px_rgba(10,31,68,0.08)]";
  ...
  const renderOverview = () => (
    <div className="animate-tab-content">
      {/* Visit Info */}
      <div className={card}>
        <h2 className="text-lg font-bold mb-2" style={{ color: "#0A1F44" }}>
          {visit.business_name}
        </h2>
        ...
        <div className="text-sm space-y-1" style={{ color: "#374151" }}>
          {visit.location && (
            <div className="flex items-center gap-1.5">📍 {visit.location}</div>
          )}
      ...
      {visit.status === "in_progress" && (
        <button
          className={`${btnPrimary} mb-3 !bg-[#2563EB]`}
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <button
          className={`${btnOutline} ${visit.start_gps ? "text-[#0A1F44] border-[#0A1F44]" : ""}`}
          onClick={() => captureGPS("start")}
        >
          {visit.start_gps ? "✓ GPS bắt đầu" : "Ghi GPS bắt đầu"}
        </button>
        <button
          className={`${btnOutline} ${visit.end_gps ? "text-[#0A1F44] border-[#0A1F44]" : ""}`}
      ...
      {visit.compliance_score != null && (
        <div className={`${card} text-center`}>
          <div
            className="w-24 h-24 rounded-full mx-auto mb-2 flex items-center justify-center text-2xl font-bold"
            style={{
              border: `6px solid ${visit.compliance_score >= 80 ? "#0A1F44" : visit.compliance_score >= 60 ? "#D97706" : "#DC2626"}`,
              color:
                visit.compliance_score >= 80
                  ? "#0A1F44"
                  : visit.compliance_score >= 60
                    ? "#D97706"
                    : "#DC2626",
            }}
          >
      ...
      {(visit.status === "in_progress" || visit.status === "completed") && (
        <div className={card}>
          <h3 className="text-base font-bold mb-3" style={{ color: "#0A1F44" }}>
            Chữ ký
          </h3>
          {(["auditor", "business"] as const).map((type) => {
            const done =
              type === "auditor"
                ? visit.has_auditor_sig
                : visit.has_business_sig;
            const ref =
              type === "auditor" ? auditorCanvasRef : businessCanvasRef;
            return (
              <div key={type} className="mb-4">
                <label className={labelCls}>
                  {type === "auditor" ? "Kiểm tra viên" : "Doanh nghiệp"}{" "}
                  {done && <span className="text-[#0A1F44]">✓ Đã ký</span>}
                </label>
                {!done && (
                  <>
                    <canvas
                      ...
                      width={300}
                      height={120}
                      className="border border-[#E2E8F0] rounded-lg w-full max-w-[300px]"
                      style={{ touchAction: "none" }}
                    />
                    <button
                      className={`${btnOutline} mt-2`}
                      onClick={() => uploadSignature(type, ref.current)}
                    >
                      Lưu chữ ký
                    </button>
                  </>
  ...
  const renderChecklist = () => (
    <div className="animate-tab-content">
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <button
          className={`${btnOutline} ${showAddItem ? "!text-red-500 !border-red-300" : "!text-[#0A1F44] !border-[#0A1F44]"}`}
          onClick={() => setShowAddItem(!showAddItem)}
        >
          {showAddItem ? "Hủy" : "+ Thêm hạng mục"}
        </button>
        {items.length === 0 && (
          <button
            className={`${btnOutline} !text-[#2563EB] !border-[#2563EB]`}
            onClick={populateChecklist}
          >
            Tạo từ template
          </button>
        )}
      ...
      {/* Add item form */}
      {showAddItem && (
        <div className="bg-[#F5F1E8] rounded-xl p-4 mb-3 border border-[#D9B96E] space-y-3 animate-modal-content">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div>
              <label className={labelCls}>Mã</label>
              <input
                className={inputCls}
                placeholder="VD: HL-01"
                value={newItem.code}
                onChange={(e) =>
          ...
          <div>
            <label className={labelCls}>Tiêu chí *</label>
            <textarea
              className={`${inputCls} resize-none`}
              rows={2}
              placeholder="Mô tả tiêu chí kiểm tra"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <div>
              <label className={labelCls}>Điều khoản</label>
              <input
                className={inputCls}
                placeholder="VD: TCVN 4.1.1"
            ...
            <div>
              ...
              <input
                className={inputCls}
                placeholder="Kiểm tra, phỏng vấn..."
                value={newItem.audit_method}
                onChange={(e) =>
                  setNewItem({ ...newItem, audit_method: e.target.value })
                }
              />
            </div>
            <div>
          ...
          <div>
            <label className={labelCls}>Tài liệu tham chiếu</label>
            <input
              className={inputCls}
              placeholder="VD: SOP vệ sinh, sổ nhật ký..."
              value={newItem.documents}
              onChange={(e) =>
                setNewItem({ ...newItem, documents: e.target.value })
              }
            />
          </div>
          <div className="flex gap-2 pt-1">
            <button
              className={`${btnPrimary} !text-sm flex-1`}
              onClick={addItem}
            >
              Thêm hạng mục
            </button>
            <button
              className={`${btnOutline} text-center flex-1`}
      ...
      {Object.entries(grouped).map(([cat, catItems]) => (
        <div key={cat} className="mb-4">
          <h3
            className="text-sm font-bold py-2 mb-2 border-b-2 border-[#E2E8F0]"
            style={{ color: "#0A1F44" }}
          >
            {cat}
          </h3>
            ...
            return (
              <div key={item.id} className={`${card} doc-card-hover`}>
                ...
                <div className="grid grid-cols-5 gap-1 mb-2.5">
                  {RESULTS.map((r) => {
                    ...
                    return (
                      <button
                        key={r.value}
                        onClick={() => setItemResult(item.id, r.value)}
                ...
                <textarea
                  ...
                  rows={1}
                  placeholder="Bằng chứng / quan sát..."
                  value={item.evidence || ""}
                  onChange={(e) =>
                    updateItemField(item.id, "evidence", e.target.value)
                  }
                />
                ...
                {(item.result === "minor_nc" || item.result === "major_nc") && (
                  ...
                  >
                    <textarea
                      className={`${inputCls} resize-none !border-red-200 mb-1.5`}
                      rows={1}
                    ...
                    <select
                      ...
                      value={item.corrective_status || "pending"}
                      onChange={(e) =>
                        updateItemField(
                          item.id,
                          "corrective_status",
                ...
                <div className="flex gap-2 mt-2 items-center">
                  <input
                    ...
                    value={item.note || ""}
                    onChange={(e) =>
                      updateItemField(item.id, "note", e.target.value)
                    }
                  />
                  <label
                    className={`${btnOutline} !px-3 !py-2 inline-flex items-center gap-1 flex-shrink-0 cursor-pointer`}
                  >
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (f) uploadItemPhoto(item.id, f);
                        e.target.value = "";
                      }}
                    />
                  </label>
  ...
  const renderNcr = () => (
    <div className="animate-tab-content">
      <button
        className={`${btnOutline} w-full mb-3 text-center !text-red-500 !border-red-300`}
        onClick={() => setShowAddNcr(!showAddNcr)}
      >
        {showAddNcr ? "Hủy" : "+ Thêm NCR"}
      </button>
        ...
        <div className="bg-red-50 rounded-xl p-4 mb-3 border border-red-200 space-y-2 animate-modal-content">
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setNewNcr({ ...newNcr, description: e.target.value })
              }
            />
          </div>
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <input
                type="date"
                className={inputCls}
                value={newNcr.deadline}
                onChange={(e) =>
                  setNewNcr({ ...newNcr, deadline: e.target.value })
                }
              />
            </div>
          </div>
          <div>
            <label className={labelCls}>Hành động khắc phục</label>
            <textarea
              className={`${inputCls} resize-none`}
              rows={1}
              value={newNcr.corrective_action}
              onChange={(e) =>
                setNewNcr({ ...newNcr, corrective_action: e.target.value })
              }
            />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <button
              className={`${btnPrimary} !bg-red-600 !text-sm`}
              onClick={addNcr}
            >
              Thêm NCR
            </button>
            <button
              className={`${btnOutline} text-center`}
              onClick={() => setShowAddNcr(false)}
            >
              Hủy
            </button>
          </div>
        ...
        <div key={ncr.id} className={`${card} doc-card-hover`}>
          ...
          {ncr.corrective_action && (
            <p className="text-xs" style={{ color: "#374151" }}>
              <strong>Khắc phục:</strong> {ncr.corrective_action}
            </p>
          )}
          {ncr.photo_paths?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-2">
              {ncr.photo_paths.map((p, i) => (
                <img
                  key={i}
                  src={p}
                  alt=""
          ...
          <label
            className={`${btnOutline} mt-2 inline-flex items-center gap-1 text-xs cursor-pointer`}
          >
            📷 Thêm ảnh
            <input
              type="file"
  ...
  const renderReport = () => (
    <div className="animate-tab-content">
      <div className={card}>
        ...
        <button className={`${btnPrimary} mb-2`} onClick={generateReport}>
          Xuất báo cáo PDF
        </button>
        {visit.report_pdf_path && (
          <button
            className={`${btnOutline} w-full text-center !text-[#0EA5E9] !border-[#0EA5E9]`}
        ...
        <div className={card}>
          ...
          <div className="grid grid-cols-3 gap-2">
            <button
              ...
              onClick={() => submitDecision("renew")}
            >
              Gia hạn
            </button>
            <button
              className={`${btnPrimary} !text-sm !bg-[#D97706]`}
              onClick={() => submitDecision("suspend")}
            >
              Tạm đình chỉ
            </button>
            <button
              className={`${btnPrimary} !text-sm !bg-[#DC2626]`}
              onClick={() => submitDecision("revoke")}
            >
              Thu hồi
            </button>
          </div>
  ...
  return (
    <div className="flex flex-col flex-1 lg:min-h-0 w-full" data-page>
      ...
      {/* Tabs */}
      <div
        className="grid grid-cols-4 gap-1 mb-4 p-1 rounded-xl"
