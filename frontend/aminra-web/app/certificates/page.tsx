...
const STATUS_BADGE: Record<string, { label: string; color: string; bg: string }> = {
  active:    { label: 'Hoạt động',   color: '#087653', bg: 'rgba(8,118,83,0.12)' },
...
export default function CertificatesPage() {
  ...
  const [stats, setStats] = useState<CertStats>({ active: 0, expiring: 0, suspended: 0, revoked: 0 });
  const [certificates, setCertificates] = useState<Certificate[]>([]);
  const [fetching, setFetching] = useState(false);
  const [tab, setTab] = useState('all');

  // Confirm dialog state
  const [confirmAction, setConfirmAction] = useState<{ id: string; action: 'suspended' | 'revoked' } | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStats(d.stats || { active: 0, expiring: 0, suspended: 0, revoked: 0 });
  ...
  const handleStatusChange = async () => {
    ...
    try {
      await fetch(`/api/api/submissions/certificates/${confirmAction.id}/status`, {
        method: 'PUT',
  ...
  const filtered = certificates.filter(c => {
    ...
    if (tab === 'expiring') return c.status === 'active' && c.days_remaining <= 90;
    return c.status === tab;
  });
  ...
  const statCards = [
    { label: 'Hoạt động', value: stats.active, color: '#087653', bg: 'rgba(8,118,83,0.12)' },
    ...
    <div className="flex flex-col flex-1 lg:min-h-0 w-full overflow-x-hidden" data-page>
      ...
      <div className="rounded-2xl p-6 mb-6 animate-section"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#1A2332' }}>Sổ đăng ký chứng nhận</h1>
            <p className="text-sm" style={{ color: '#5F6F80' }}>{certificates.length} chứng nhận · {user.company_name}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6 animate-section">
        {statCards.map((s, i) => (
          <div key={i} className="rounded-xl p-4"
            ...
            <p className="text-xs font-medium mb-1" style={{ color: '#5F6F80' }}>{s.label}</p>
      ...
      <div className="flex-1 lg:min-h-0 lg:overflow-y-auto space-y-3">
        {fetching ? (
          ...
          <div className="rounded-2xl p-12 text-center animate-section" style={{ background: '#F0F7F4', border: '1px solid #E2E8F0' }}>
            ...
            <p className="font-semibold mb-2" style={{ color: '#1A2332' }}>Không có chứng nhận nào</p>
            <p className="text-sm" style={{ color: '#5B6B7D' }}>Chưa có chứng nhận nào phù hợp với bộ lọc hiện tại</p>
          </div>
        ) : (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <p className="text-sm font-semibold" style={{ color: '#1A2332' }}>{cert.company_name}</p>
                      <span className="px-2 py-0.5 rounded-lg text-xs font-medium"
                        style={{ background: badge.bg, color: badge.color }}>
                        {badge.label}
                      </span>
                    </div>
                    <div className="flex items-center gap-4 text-xs flex-wrap" style={{ color: '#5B6B7D' }}>
                      <span className="font-mono" style={{ color: '#94A3B8' }}>#{cert.cert_number}</span>
                      <span>Cấp: {formatDate(cert.issue_date)}</span>
                      <span>Hết hạn: {formatDate(cert.expiry_date)}</span>
                      {cert.status === 'active' && (
                        <span className="font-semibold" style={{ color: daysColor(cert.days_remaining) }}>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <a href={`/api/api/submissions/certificates/${cert.id}/pdf?token=${token}`}
                    ...
                    {cert.status === 'active' && (
                      <>
                        <button
                          onClick={() => setConfirmAction({ id: cert.id, action: 'suspended' })}
                          className="px-3 py-1.5 rounded-lg text-xs font-medium transition-all hover:scale-105"
                          style={{ background: 'rgba(217,119,6,0.1)', color: '#D97706', border: '1px solid rgba(217,119,6,0.2)' }}>
                          Đình chỉ
                        </button>
                        <button
                          onClick={() => setConfirmAction({ id: cert.id, action: 'revoked' })}
                          className="px-3 py-1.5 rounded-lg text-xs font-medium transition-all hover:scale-105"
      ...
      {confirmAction && (
        <div className="fixed inset-0 z-50 grid place-items-center p-4 animate-modal-overlay"
          ...
          <div className="w-full max-w-md rounded-2xl p-6 animate-modal-content"
            ...
            <p className="text-sm mb-4" style={{ color: '#5F6F80' }}>
            ...
            <div className="flex gap-3">
              <button onClick={() => { setConfirmAction(null); setReason(''); }}
                ...
                style={{ background: '#F0F7F4', color: '#5F6F80', border: '1px solid #E2E8F0' }}>
                Hủy
              </button>
              <button onClick={handleStatusChange} disabled={!reason.trim() || actionLoading}
                ...
                style={{
                  background: (!reason.trim() || actionLoading) ? '#E2E8F0' : confirmAction.action === 'suspended' ? '#D97706' : '#DC2626',
                  color: (!reason.trim() || actionLoading) ? '#5B6B7D' : 'white',
                }}>
                {actionLoading ? 'Đang xử lý...' : confirmAction.action === 'suspended' ? 'Xác nhận đình chỉ' : 'Xác nhận thu hồi'}
              </button>
            </div>
          </div>
        </div>
