...
describe('VerifyPage — blockchain section', () => {
  test('shows pending message when blockchain.anchored=false', async () => {
    (fetch as unknown as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      respond(200, {
        ...baseCert,
        blockchain: { anchored: false, note: 'Anchor pending — within 24 hours.' },
      })
    );
    ...
    expect(screen.queryByText(/Verify on PolygonScan/i)).not.toBeInTheDocument();
  });
  ...
  test('shows Polygon badge + tx hash + PolygonScan link when anchored', async () => {
    ...
    expect(link).toHaveAttribute('href', 'https://polygonscan.com/tx/0x' + '9e7c'.repeat(16));
  ...
  test('shows Bitcoin badge when chain=bitcoin', async () => {
    (fetch as unknown as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      respond(200, {
        ...
        blockchain: {
          ...
          explorer_url: 'https://www.blockchain.com/btc/tx/abc' + 'd'.repeat(61),
        },
      })
    );
    renderPage('HALAL-2026-0042');
    ...
    expect(screen.getByRole('link', { name: /Verify on Blockchain.com/i })).toBeInTheDocument();
  });
  ...
  test('cert without blockchain field still renders (backwards compat)', async () => {
    ...
    expect(screen.queryByText(/Blockchain verification/i)).not.toBeInTheDocument();
  });
