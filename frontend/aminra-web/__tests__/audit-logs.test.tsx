...
describe("AuditLogsPage", () => {
  test("shows login prompt when no token in storage", async () => {
    ...
    expect(
      await screen.findByText(/cần đăng nhập với tài khoản admin/i),
    ).toBeInTheDocument();
  ...
  test("apply filters re-fetches with filter params + resets page to 1", async () => {
    ...
    fetchMock
      .mockResolvedValueOnce(
        respond(200, { logs: [], total: 0, page: 1, limit: 25 }),
      ) // initial
      .mockResolvedValueOnce(
        respond(200, { logs: [], total: 0, page: 1, limit: 25 }),
      ); // after filter
    ...
    fireEvent.change(screen.getByPlaceholderText("login.success"), {
      target: { value: "login.failed" },
    });
  ...
  test("shows anonymous indicator for logs without user_email", async () => {
    ...
    (fetch as unknown as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      respond(200, {
        logs: [
          mkLog({ user_email: null, user_role: null, action: "login.failed" }),
        ],
        total: 1,
        page: 1,
        limit: 25,
