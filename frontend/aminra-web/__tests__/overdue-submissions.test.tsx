...
describe('OverdueSubmissionsPage', () => {
  ...
  test('count summary text uses item count', async () => {
    ...
    (fetch as unknown as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      respond(200, {
        items: [{
          submission_id: 'x', company_name: 'A', status: 'reviewing',
          submitted_at: null, deadline: '2026-01-01T00:00:00Z',
          provider_email: null, provider_name: null, days_overdue: 1.0,
        }],
        count: 1,
      }),
    );
    ...
    render(<OverdueSubmissionsPage />);
    expect(await screen.findByText(/1 hồ sơ đã quá deadline/i)).toBeInTheDocument();
  });
});
