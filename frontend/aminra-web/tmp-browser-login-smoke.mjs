...
const outRoot = path.resolve('../../docs/qa/2026-09-07-login-all-account-types');
const outDir = path.join(outRoot, 'evidence/screenshots');
await fs.mkdir(outDir, { recursive: true });

const accounts = [
  { label: 'business', start: '/business/login', username: 'biz-demo-1@demo.aminra.vn', password: DEMO_PW, expectUrl: /dashboard\/business|auth\/callback/ },
  { label: 'cb_provider', start: '/provider/login', username: 'cb-demo@demo.aminra.vn', password: DEMO_PW, expectUrl: /dashboard\/provider|auth\/callback/ },
...
await fs.writeFile(path.join(outRoot, 'browser-login-smoke-results.json'), JSON.stringify(out, null, 2));
