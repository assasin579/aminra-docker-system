...
const BIZ_LOGIN  = { email: "biz-demo-1@demo.aminra.vn",  password: "DemoP@ss2026", role: "business" };
const PROV_LOGIN = { email: "cb-demo@demo.aminra.vn",     password: "DemoP@ss2026", role: "provider" };
...
test.describe("Phase 1 Batch 1 — workflow integrity", () => {
  ...
  test("static guards — code reflects all batch 1 fixes", async () => {
    ...
    const src = await readFile("../../backend/auth/submission_router.py", "utf8");

    // C1: replace rejects approved/rejected
    expect(src).toMatch(/sub\["status"\] in \(['"]approved['"], ['"]rejected['"]\)/);
    ...
    expect(src).toMatch(/UPDATE submissions SET status='approved'.*WHERE id=\$1 AND status <> 'approved' RETURNING id/s);
    ...
    expect(src, "finalize must NOT delete submission").not.toMatch(/DELETE FROM submissions WHERE id = \$1/);
  ...
  test("DB-level: status CHECK constraint blocks garbage values", async ({ request }) => {
    ...
    const r = await request.put(`/api/api/submissions/received/${target.id}/status`, {
  ...
  test("approve-final rejects when no evaluation saved (W3-M11)", async ({ request }) => {
    const login = await request.post("/api/auth/login", { data: PROV_LOGIN });
    const { access_token } = await login.json();
    const list = await request.get("/api/api/submissions/received", {
      headers: { Authorization: `Bearer ${access_token}` },
    ...
    const r = await request.post(`/api/api/submissions/received/${target.id}/approve-final`, {
      headers: { Authorization: `Bearer ${access_token}` },
    });
  ...
  test("audit_logs trigger blocks UPDATE/DELETE (decision #4)", async ({ request }) => {
    ...
    const auditMutators = paths.filter(p =>
