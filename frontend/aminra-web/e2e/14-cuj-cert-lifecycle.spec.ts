...
test.describe("CUJ-10: Cert lifecycle", () => {
  test("Public verify endpoint returns 404 for unknown cert", async ({
    api,
  }) => {
    const res = await api.get(
      "/api/submissions/certificates/public/HALAL-NOT-EXIST-2026",
    );
    expect(res.status()).toBe(404);
  });

  test("Verify page shows 'không tìm thấy' for unknown cert", async ({
    page,
  }) => {
  ...
  test("Revocation requires reason (validation enforced)", async ({
    api,
    biz,
  }) => {
  ...
  test("Cert page (auth-required) renders for provider", async ({
    page,
    prov,
  }) => {
