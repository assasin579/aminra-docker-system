...
export default function HomePage() {
  ...
  const handleSend = async () => {
    ...
    try {
      const apiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8000'}/chat/stream`;
  ...
  const whyItems = [
    {
      ...
      titleKey: 'home.why1_title', descKey: 'home.why1_desc', color: '#16a34a', bg: 'rgba(22,163,74,0.15)'
    },
    {
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      titleKey: 'home.why2_title', descKey: 'home.why2_desc', color: '#3b82f6', bg: 'rgba(59,130,246,0.15)'
    },
    {
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="flex flex-col flex-1 lg:min-h-0 max-w-7xl mx-auto w-full">
      ...
      {/* Chat Container */}
      <div className="flex-1 lg:min-h-0 rounded-2xl shadow-xl overflow-hidden flex flex-col" style={{background:'#162847', border:'1px solid #1e3a5f'}}>
        {/* Chat Header */}
        <div className="p-4" style={{borderBottom:'1px solid #1e3a5f', background:'#162847'}}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="relative flex-shrink-0">
                <div className="h-11 w-11 rounded-full overflow-hidden ring-2 ring-green-500/50" style={{animation:'pulse-ring 2.5s ease-in-out infinite'}}>
                  <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
                    {/* App bg fill */}
                    <circle cx="50" cy="50" r="50" fill="#162847"/>
                    {/* Blue coat - bottom 35% */}
                ...
                <div className="absolute bottom-0 right-0 h-3 w-3 rounded-full bg-green-400" style={{outline:'2px solid #162847', animation:'blink 1.8s ease-in-out infinite'}}></div>
              </div>
              <div>
                <h2 className="text-base font-semibold text-white leading-tight">{t('home.chat_title')}</h2>
                <p className="text-xs text-green-400 flex items-center gap-1">
                  <span className="inline-block h-1.5 w-1.5 rounded-full bg-green-400" style={{animation:'blink 1.8s ease-in-out infinite'}}></span>
                  {t('home.online')}
                </p>
              </div>
            </div>
            ...
            {availableVoices.length > 1 && (
              <div className="relative">
                <button
                  ...
                  style={{border:'1px solid #1e3a5f', background:'#111725'}}
                >
                ...
                {showVoicePicker && (
                  <div className="absolute right-0 top-full mt-1 z-50 rounded-xl shadow-xl overflow-hidden min-w-[220px]"
                    ...
                    <div className="px-3 py-2 text-xs text-slate-400" style={{borderBottom:'1px solid #1e3a5f'}}>
                    ...
                    {availableVoices.map(v => (
                      <button key={v.name} onClick={() => { setSelectedVoiceName(v.name); setShowVoicePicker(false); }}
                        className={`w-full text-left px-3 py-2.5 text-sm transition-colors flex items-center justify-between gap-2 ${selectedVoiceName === v.name ? 'text-green-400 bg-green-600/10' : 'text-slate-300 hover:bg-white/5'}`}>
                        <span className="truncate">{v.name}</span>
                        {!v.localService && <span className="text-xs text-green-500 flex-shrink-0">online</span>}
                      </button>
                    ))}
                  </div>
                )}
        ...
        {/* Messages Area */}
        <div className="relative flex-1 lg:min-h-0 min-h-[300px] flex flex-col" style={{background:'#111725'}}>
        {showScrollTop && (
          <button
            onClick={() => {
        ...
        <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div
                className={`max-w-[80%] rounded-2xl p-4 shadow-sm ${msg.sender === 'user' ? 'bg-green-600 text-white' : 'text-slate-100'}`}
                style={msg.sender === 'user' ? {} : {background: speakingId === msg.id ? '#1a3a5c' : '#162847', border:`1px solid ${speakingId === msg.id ? '#22c55e' : '#1e3a5f'}`}}
              >
                ...
                <div className="whitespace-pre-wrap">
                  {msg.text}
                  {streamingId === msg.id && (
                    <span className="inline-block w-0.5 h-4 bg-green-400 ml-0.5 align-middle" style={{animation:'blink 0.8s ease-in-out infinite'}} />
                  )}
                </div>
                {speakingId === msg.id && (
                  <div className="flex items-center gap-1 mt-2">
                    {[0, 0.15, 0.3, 0.15, 0].map((delay, i) => (
                      <span key={i} className="inline-block w-0.5 bg-green-400 rounded-full"
                        style={{height:`${8 + i * 3}px`, animation:'soundwave 0.8s ease-in-out infinite', animationDelay:`${delay}s`}}
                      />
                    ))}
                    <span className="text-xs text-green-400 ml-1">Đang đọc...</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="rounded-2xl p-4" style={{background:'#162847', border:'1px solid #1e3a5f'}}>
        ...
        {/* Input Area */}
        <div className="p-6" style={{borderTop:'1px solid #1e3a5f', background:'#162847'}}>
          <div className="flex items-end gap-3">
            <div className="flex-1 relative">
              <textarea
                ref={inputRef}
                className="w-full rounded-xl p-4 focus:ring-2 focus:ring-green-500 focus:outline-none resize-none text-white"
                style={{background:'#111725', border:`1px solid ${isListening ? '#22c55e' : '#1e3a5f'}`}}
            ...
            <div className="flex flex-col gap-2">
              {voiceSupported && (
                <button
                  ...
                  className={`p-3 rounded-xl transition-all ${isListening ? 'bg-red-500 hover:bg-red-400 shadow-lg shadow-red-500/30' : 'hover:bg-green-600/20'}`}
                  style={isListening ? {} : {border:'1px solid #1e3a5f', background:'#111725'}}
                >
                  {isListening ? (
              ...
              <button
                ...
                className="px-8 py-4 bg-green-600 hover:bg-green-500 text-white font-semibold rounded-xl disabled:opacity-50 transition-all shadow-md"
              >
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {(['preset1', 'preset2', 'preset3', 'preset4'] as const).map((key) => (
              <button
                ...
                style={{background:'#111725', border:'1px solid #1e3a5f', color:'#7eb8e8'}}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#22c55e'; (e.currentTarget as HTMLElement).style.color = '#22c55e'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = '#1e3a5f'; (e.currentTarget as HTMLElement).style.color = '#7eb8e8'; }}
              >
                {t(`home.${key}`)}
              </button>
            ))}
          </div>
          <div className="mt-2 text-xs" style={{color:'#4a7aa8'}}>
            {t('home.enter_hint')}
          </div>
        </div>
      </div>
      ...
      {/* Why Choose Mukjizat Saigoncert */}
      <div className="lg:w-80 rounded-2xl overflow-hidden flex flex-col lg:min-h-0" style={{background:'#162847', border:'1px solid #1e3a5f'}}>
        <div className="px-5 pt-5 pb-3">
          <h3 className="text-lg font-bold text-white">{t('home.why_title')}</h3>
          <div className="h-0.5 mt-3 rounded-full" style={{background:'linear-gradient(to right, #16a34a, transparent)'}}></div>
        </div>
          ...
          {whyItems.map((item) => (
            ...
            >
              ...
              <div className="min-w-0">
                <div className="text-sm font-semibold text-white leading-tight">{t(item.titleKey)}</div>
                <div className="text-xs mt-0.5 leading-relaxed" style={{color:'#6b8fad'}}>{t(item.descKey)}</div>
              </div>
            </div>
          ))}
        </div>
