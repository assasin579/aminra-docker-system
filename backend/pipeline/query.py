...
def enhance_query_for_retrieval(text: str) -> Dict[str, Any]:
    ...
    strategies = {
        ...
        "requirement": {"boost_title": 1.3, "min_score": 0.07, "require_exact_match": True},
        "general": {"boost_title": 1.0, "min_score": 0.08, "require_exact_match": False},
    }
    ...
    return {"original": text, "type": query_type, "expanded": expanded_queries, "strategy": strategy}
...
def embed_query(text):
    ...
    return {"vector": encoder.encode(formatted_query, normalize_embeddings=True).tolist(), "enhanced": enhanced}
...
def hybrid_search(query_data, top_k=TOP_K, topic_filter=None):
    ...
    for expanded_query in enhanced["expanded"]:
        try:
            ...
            title_filter = Filter(must=[FieldCondition(key="slide_title", match=MatchText(text=expanded_query))])
            ...
            text_filter = Filter(must=[FieldCondition(key="text", match=MatchText(text=expanded_query))])
...
def call_openrouter(prompt_text, original_question=None, context=None):
    ...
    try:
        response = session.post(f"{OPENROUTER_BASE_URL}/chat/completions", headers=headers, json=payload, timeout=45)
        ...
        log.error("⏱️ OpenRouter timeout - fallback to DeepSeek")
        return call_deepseek(prompt_text, original_question, context)
