...
MODELS = {
    "deepseek": {"id": "deepseek/deepseek-chat", "name": "DeepSeek Chat", "cost": "cheap", "speed": "fast"},
    "llama": {"id": "meta-llama/llama-2-70b-chat", "name": "Llama 2 70B", "cost": "cheap", "speed": "fast"},
    "mixtral": {"id": "mistralai/mixtral-8x7b", "name": "Mixtral 8x7B", "cost": "cheap", "speed": "fast"},
    "claude": {"id": "anthropic/claude-3-haiku", "name": "Claude 3 Haiku", "cost": "moderate", "speed": "fast"},
}
...
class OpenRouterClient:
    ...
    def call_model(
        ...
        payload = {
            "model": model_id,
            "messages": [{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": prompt[:6000]}],
        ...
        try:
            ...
            response = self.session.post(
                f"{OPENROUTER_BASE_URL}/chat/completions", headers=self._get_headers(), json=payload, timeout=timeout
            )
            ...
            return {"success": False, "model": model_config["name"], "error": "request_error", "message": str(e)}
        except Exception as e:
            log.error(f"❌ Unexpected error with {model_config['name']}: {e}")
            return {"success": False, "model": model_config["name"], "error": "unexpected", "message": str(e)}

    def call_with_fallback(
        self, prompt: str, preferred_model: Optional[str] = None, fallback_models: Optional[list] = None, **kwargs
    ) -> Dict[str, Any]:
...
def call_openrouter(
    ...
    try:
        ...
        if use_fallback:
            result = client.call_with_fallback(prompt, preferred_model=preferred_model, fallback_models=FALLBACK_MODELS)
        else:
