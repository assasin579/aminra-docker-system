        ...
        "auditor_workload": [
            {
                "id": str(a["id"]),
                "name": a["company_name"],
                "active_visits": a["active_visits"],
                "active_submissions": a["active_submissions"],
            }
            for a in auditors
        ],
        "expiring_certs": [
            {
                "cert_number": e["cert_number"],
...
async def list_providers(db: Connection = Depends(get_db)):
    ...
    return {"providers": [{"id": str(r["id"]), "company_name": r["company_name"], "email": r["email"]} for r in rows]}
...
async def request_renewal(
    ...
    orig_sub = (
        await db.fetchrow("SELECT provider_id, document_ids FROM submissions WHERE id=$1", cert["submission_id"])
    ...
    if not doc_ids:
        raise HTTPException(
            400, "Không tìm thấy tài liệu của chứng nhận gốc — vui lòng upload tài liệu mới rồi tạo hồ sơ mới"
        )
    ...
    """,
        ...
        provider_id,
        doc_ids,
        f"Yêu cầu gia hạn chứng nhận {cert['cert_number']}",
        company_name,
    )
    ...
    await notify(
        ...
        f"Chứng nhận hết hạn: {cert['expiry_date']}",
        "/submissions",
    )
...
async def assign_auditor(
    ...
    await notify(
        ...
        "Vui lòng xem và đánh giá hồ sơ",
        "/submissions",
    )
...
async def get_evaluation(
    ...
    return {
        "evaluation": {
            ...
            if isinstance(row["checklist"], list)
            else (_json.loads(row["checklist"]) if isinstance(row["checklist"], str) else []),
            "score": row["score"],
            "notes": row["notes"],
    ...
    if sub_info:
        ...
        if str(sub_info["provider_id"]) != user["sub"]:
            await notify(
                ...
                f"{score_text}. {req.notes[:80] if req.notes else ''}",
                "/submissions",
            )
...
async def replace_submission_document(
    ...
    if sub["status"] == "revision_required":
        ...
        await db.execute(
            "UPDATE submissions SET document_ids = $1::uuid[], updated_at = NOW() WHERE id = $2", doc_ids, submission_id
        )

    # Notify provider + auditor
    sub_info = await db.fetchrow(
        "SELECT provider_id, auditor_id, company_name FROM submissions WHERE id=$1", submission_id
    )
    if sub_info:
        await notify(
            db,
            ...
            "Phiên bản mới đã được upload",
            "/submissions",
        )
        if sub_info["auditor_id"]:
            await notify(
                db,
                str(sub_info["auditor_id"]),
                "status_change",
                f"{sub_info['company_name']} đã cập nhật tài liệu",
                "Phiên bản mới đã được upload",
                "/submissions",
