...
async def issue_certificate_for_company(
    ...
    active_subs = await db.fetch(
        ...
        business_tenant_id, provider_id)
    if not active_subs:
    ...
    if not_approved:
        raise HTTPException(400, f"Còn {len(not_approved)} hồ sơ chưa được duyệt. Phải duyệt tất cả hồ sơ đang hoạt động trước khi cấp chứng nhận.")
    ...
    biz_user = await db.fetchrow("SELECT company_name FROM users WHERE (id=$1 OR tenant_id=$1) AND is_owner=true LIMIT 1", business_tenant_id)
    ...
    for attempt in range(5):
        ...
        try:
            ...
            """, candidate, user["sub"], business_tenant_id, business_name,
                issue_date, expiry_date, "", req.notes)
...
async def public_verify_cert(cert_number: str, db: Connection = Depends(get_db)):
    ...
    response = {
        ...
        "revocation": (
            ...
            if row.get("revoked_at") else None
        ),
    }
    ...
    if proof_row:
        ...
        explorer_url = (
            f"https://polygonscan.com/tx/{tx_hash}" if chain == "polygon"
            else f"https://www.blockchain.com/btc/tx/{tx_hash}" if chain == "bitcoin"
            else None
        )
        response["blockchain"] = {
            "anchored": True,
...
async def update_cert_status(
    ...
    if new_status == "revoked":
        ...
        from services.cert_lifecycle import (
            AlreadyRevoked, InvalidRevocation, revoke_cert,
        )
        try:
            await revoke_cert(
                db,
