...
async def list_documents(
    ...
    rows = await db.fetch(
        ...
        SELECT l.id, l.original_filename, l.doc_type, l.compliance_score,
               l.file_size, l.mime_type, l.uploaded_at, l.evaluation_result,
...
async def preview_document(
    ...
    if user["role"] == "business":
        ...
        # Provider can view docs from submissions assigned to them
        row = await db.fetchrow(
            """SELECT d.file_path, d.original_filename, d.mime_type FROM documents d
               WHERE d.id = $1 AND EXISTS (
                   SELECT 1 FROM submissions s WHERE $1 = ANY(s.document_ids) AND s.provider_id = $2
               )""",
            doc_id, user["sub"],
        )
    if not row:
    ...
    # Use cached version if source file hasn't changed
    if cache_pdf.exists() and cache_pdf.stat().st_mtime >= file_path.stat().st_mtime:
        from fastapi.responses import FileResponse
    ...
    try:
        result = subprocess.run(
            ["libreoffice", "--headless", "--convert-to", "pdf",
             "--outdir", str(cache_dir), str(file_path)],
            capture_output=True, timeout=60,
        )
        # LibreOffice outputs to {outdir}/{original_stem}.pdf
        lo_output = cache_dir / (file_path.stem + ".pdf")
        if lo_output.exists():
...
async def get_document_file(
    doc_id: str,
    token: Optional[str] = Query(None),  # Accept token via query param for iframe/download
    user=Depends(get_current_user),
    db=Depends(get_db),
):
...
async def delete_document(
    ...
    if not user.get("is_owner"):
        raise HTTPException(403, "Chỉ chủ tài khoản mới có thể xoá tài liệu")
