...
async def list_processes(user=Depends(get_current_user), db=Depends(get_db)):
    ...
    rows = await db.fetch(
        "SELECT * FROM process_templates WHERE tenant_id=$1 ORDER BY updated_at DESC",
        tenant_id)
    return {"processes": [
        ProcessOut(
...
async def create_process(req: ProcessCreate, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    """, tenant_id, req.name, req.description, flowchart)
...
async def get_process(pid: str, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    tenant_id = _require_business(user)
    row = await db.fetchrow(
        "SELECT * FROM process_templates WHERE id=$1 AND tenant_id=$2", pid, tenant_id)
    if not row:
    ...
    return ProcessOut(
        ...
        flowchart=fc, version=row["version"], is_active=row["is_active"],
...
async def update_process(pid: str, req: ProcessUpdate, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    if req.flowchart is not None:
        updates.append(f"flowchart = ${idx}::jsonb"); params.append(_json.dumps(req.flowchart)); idx += 1
...
async def export_process_docx(pid: str, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    sorted_nodes = sorted(
        [n for n in nodes if n.get("type") == "main"],
    ...
    if sorted_nodes:
        ...
        for i, node in enumerate(sorted_nodes, 1):
            ...
            parallel_targets = [e["to"] for e in edges if e.get("from") == node.get("id") and e.get("type") == "parallel"]
            if parallel_targets:
                p = doc.add_paragraph()
