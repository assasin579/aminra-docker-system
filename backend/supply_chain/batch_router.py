...
async def list_batches(
    ...
    if status:
        cond += f" AND b.status = ${idx}"; params.append(status); idx += 1
    if product:
        cond += f" AND b.product_name ILIKE ${idx}"; params.append(f"%{product}%"); idx += 1
    if period == "today":
    ...
    return {"batches": [
        BatchOut(
            id=str(r["id"]), batch_code=r["batch_code"], product_name=r["product_name"],
            process_template_id=str(r["process_template_id"]) if r["process_template_id"] else None,
            process_name=r["process_name"], status=r["status"],
            started_at=r["started_at"], completed_at=r["completed_at"],
            compliance_score=r["compliance_score"], qr_code_url=r["qr_code_url"],
            notes=r["notes"], step_count=r["step_count"], step_completed=r["step_completed"],
            material_count=r["material_count"], created_at=r["created_at"],
        ) for r in rows
    ]}
...
async def create_batch(req: BatchCreate, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    """, tenant_id, batch_code, req.product_name, process_id, req.notes)
    ...
    if req.materials:
        for m in req.materials:
            ...
            await db.execute("""
                ...
                VALUES ($1,$2,$3,$4)
            """, batch_id, m["material_id"], m.get("quantity"), m.get("unit"))

    # Auto-create steps from process template
    if process_id:
        pt = await db.fetchrow("SELECT flowchart FROM process_templates WHERE id=$1 AND tenant_id=$2", process_id, tenant_id)
        if pt and pt["flowchart"]:
            ...
            for node in sorted(nodes, key=lambda n: n.get("order", 0)):
                checklist = _json.dumps(
                    [{"text": item, "checked": False} for item in (node.get("checklist") or [])]
                )
                await db.execute("""
                    INSERT INTO batch_steps (batch_id, node_id, step_name, checklist)
                    VALUES ($1,$2,$3,$4::jsonb)
                """, batch_id, node["id"], node.get("label", "Bước"), checklist)
...
async def batch_stats(user=Depends(get_current_user), db=Depends(get_db)):
    ...
    completed = await db.fetchval("SELECT COUNT(*) FROM production_batches WHERE tenant_id=$1 AND status='completed'", tenant_id)
    in_progress = await db.fetchval("SELECT COUNT(*) FROM production_batches WHERE tenant_id=$1 AND status='in_progress'", tenant_id)
    draft = await db.fetchval("SELECT COUNT(*) FROM production_batches WHERE tenant_id=$1 AND status='draft'", tenant_id)
...
async def list_tenant_members(user=Depends(get_current_user), db=Depends(get_db)):
    ...
    rows = await db.fetch(
        ...
        tenant_id)
    return {"members": [
        {"id": str(r["id"]), "name": r["company_name"], "email": r["email"],
         "is_owner": r["is_owner"], "role": r["ihc_role"] or ("Chủ tài khoản" if r["is_owner"] else "Thành viên")}
        for r in rows
    ]}
...
async def public_trace(batch_code: str, db=Depends(get_db)):
    ...
    if row.get("integrity_hash"):
        ...
        raw = await db.fetchval(
            "SELECT sealed_data::text FROM production_batches WHERE id=$1", bid)
        if raw:
...
async def assign_member_to_batch(bid: str, request: Request, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    batch = await db.fetchrow("SELECT id, integrity_hash FROM production_batches WHERE id=$1 AND tenant_id=$2", bid, tenant_id)
    ...
    await db.execute(
        ...
        member_id, member["company_name"], bid)
...
async def get_batch(bid: str, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    # Get steps
    steps = await db.fetch(
        "SELECT * FROM batch_steps WHERE batch_id=$1 ORDER BY created_at", bid)
...
async def update_batch(bid: str, req: BatchUpdate, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    if req.product_name is not None:
        updates.append(f"product_name = ${idx}"); params.append(req.product_name); idx += 1
    if req.status is not None:
        updates.append(f"status = ${idx}"); params.append(req.status); idx += 1
...
async def update_step(bid: str, step_id: str, req: BatchStepUpdate, user=Depends(get_current_user), db=Depends(get_db)):
    _validate_uuid(bid); _validate_uuid(step_id)
    tenant_id = _require_business(user)
    await check_permission_db(user, "can_edit")

    # Verify batch belongs to tenant + not sealed
    b = await db.fetchrow("SELECT id, integrity_hash FROM production_batches WHERE id=$1 AND tenant_id=$2", bid, tenant_id)
    ...
    if req.performed_by is not None:
        updates.append(f"performed_by = ${idx}"); params.append(req.performed_by); idx += 1
    if req.notes is not None:
        updates.append(f"notes = ${idx}"); params.append(req.notes); idx += 1
    if req.checklist is not None:
        updates.append(f"checklist = ${idx}::jsonb"); params.append(_json.dumps(req.checklist)); idx += 1
...
async def approve_step(bid: str, step_id: str, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    await db.execute(
        "UPDATE batch_steps SET approved_by=$1, approved_at=NOW() WHERE id=$2",
        approver, step_id)
...
async def approve_batch(bid: str, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    await check_permission_db(user, "can_approve")
    row = await db.fetchrow(
        "SELECT * FROM production_batches WHERE id=$1 AND tenant_id=$2", bid, tenant_id)
    ...
    await db.execute("""
        ...
        WHERE id=$4
    """, approver, integrity_hash, sealed_json, bid)
...
async def verify_batch_integrity(bid: str, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    tenant_id = _require_business(user)
    row = await db.fetchrow(
        "SELECT integrity_hash, sealed_data::text AS sealed_text FROM production_batches WHERE id=$1 AND tenant_id=$2", bid, tenant_id)
    if not row: raise HTTPException(404)
...
async def export_batch_pdf(bid: str, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    if mats:
        ...
        for m in mats:
            mat_data.append([
                m['material_name'], m.get('sku') or '', m['supplier_name'] or '',
                str(m['quantity'] or ''), m.get('unit') or '',
            ])
    ...
    if steps:
        ...
        for i, s in enumerate(steps, 1):
            step_data.append([
                str(i), s['step_name'], s.get('performed_by') or '',
...
async def upload_step_photo(bid: str, step_id: str, file: UploadFile = File(...), user=Depends(get_current_user), db=Depends(get_db)):
    _validate_uuid(bid); _validate_uuid(step_id)
    tenant_id = _require_business(user)
