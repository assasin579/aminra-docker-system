...
def build(doc: DocxDocument, content: str, title: str, filename: str, cfg: dict = {}):
    ...
    B.add_cover(
        doc,
        doc_type_label="Ingredient & Raw Material",
        title=title,
    ...
    for ci, h in enumerate(["Nhà cung cấp", "Nguyên liệu", "Chứng nhận Halal", "Hạn hiệu lực"]):
        cell = chk_tbl.cell(0, ci)
        cell.text = ""
        r = cell.paragraphs[0].add_run(h)
        B.font(r, size=B.SZ_TINY, bold=True, color=B.CLR_WHITE)
        B.shade_cell(cell, "0D47A1")
        B.cell_padding(cell)
    for ci in range(4):
        cell = chk_tbl.cell(1, ci)
        cell.text = ""
        r = cell.paragraphs[0].add_run("[Điền thông tin]")
        B.font(r, size=B.SZ_TINY, italic=True, color=B.CLR_GRAY)
        B.cell_padding(cell)
    doc.add_paragraph()
